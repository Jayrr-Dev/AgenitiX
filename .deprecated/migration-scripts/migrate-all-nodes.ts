...
function generateNodeComponent(meta: NodeMeta): string {
	...
	const handlesArray = meta.handles
		.map(
			(handle) =>
...
async function migrateNode(nodeDir: string): Promise<MigrationResult> {
	...
	try {
		...
		const filesToDelete = [path.join(nodeDir, "meta.json")];
		...
		for (const migration of migrations) {
			if (migration.success) {
				const pascalCaseType =
					migration.nodeType.charAt(0).toUpperCase() + migration.nodeType.slice(1);
				const exportPath =
					migration.files.created[0]
						?.replace("features/business-logic-modern/node-domain/", "./")
						.replace(".tsx", "") || "";
				if (exportPath) {
...
async function migrateAllNodes(): Promise<void> {
	...
	if (successful.length > 0) {
		...
		successful.forEach((m) => {
			console.log(
				`  - ${m.nodeType}: ${m.files.created.length} files created, ${m.files.deleted.length} files deleted`
			);
		});
	}
