...
function SlideshowButtonNode({ id, data }: { id: string; data: any }) {
	...
	return (
		<div
			className={`px-4 py-2 shadow-lg rounded-lg bg-white border-2 transition-all duration-500 ${
				isHighlighted
					? "border-amber-500 shadow-amber-200 shadow-xl scale-105 ring-2 ring-amber-400"
					: "border-blue-600"
			}`}
		>
	...
	return (
		...
		>
			...
			<div className="text-center p-2 rounded border border-gray-300 bg-gray-50">
				<div
					className={`text-lg font-bold transition-all duration-500 ${isHighlighted ? "animate-bounce" : ""} ${
						currentValue === true
							? "text-green-800"
							: currentValue === false
								? "text-red-800"
				...
				>
					{currentValue !== undefined ? `Received: ${typeof currentValue}` : "Waiting for input..."}
				</div>
			</div>
			...
			<div className="flex h-[calc(100vh-120px)]">
				...
				<div className="w-96 bg-white border-l border-gray-300 p-6 overflow-y-auto">
					...
					<div className="mb-6">
						...
						<p className="text-gray-800 leading-relaxed font-medium">{step.description}</p>
					</div>

					{/* Code snippet */}
					<div className="mb-6">
						<h3 className="text-sm font-bold text-gray-900 mb-2">📝 Code at this step:</h3>
