...
async function autoVersion() {
	try {
		...
		try {
			...
			console.log("✅ No automatic version bump (TypeScript compilation needed)");
			console.log('💡 Run "pnpm version:init" to initialize the versioning system');
			return;
		}
		...
		console.log(`🚀 Auto-bumping version: ${changes.bumpType} (${changes.version})`);
		...
		if (changes.changedFiles.length > maxFiles) {
			console.log(`   ... and ${changes.changedFiles.length - maxFiles} more files`);
		}
