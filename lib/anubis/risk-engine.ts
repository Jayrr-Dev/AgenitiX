...
async function checkIPReputation(ip: string): Promise<number> {
	...
	try {
		const { checkIPReputation } = await import("./threat-intelligence");
...
function analyzeRequestPattern(request: any): number {
	let riskScore = 0;
...
function analyzeSessionHistory(history: any): number {
	if (!history) {
