 ...
 */
function detectSidebarIntegration(kind: string, category: string): NodeDocData['infrastructure']['sidebar'] {
  // Check if node is in the sidebar registry
  const sidebarRegistryPath = path.join(process.cwd(), 'features/business-logic-modern/infrastructure/node-registry/nodespec-registry.ts');
  
  if (fs.existsSync(sidebarRegistryPath)) {
    const registryContent = fs.readFileSync(sidebarRegistryPath, 'utf-8');
    const isRegistered = registryContent.includes(`createText: createTextSpec`) || 
...
function detectInspectorIntegration(kind: string): NodeDocData['infrastructure']['inspector'] {
  ...
  const inspectorServicePath = path.join(process.cwd(), 'features/business-logic-modern/infrastructure/node-inspector/services/NodeInspectorService.ts');
  
  let hasControls = false;
  let controlTypes: string[] = [];
  
  if (fs.existsSync(inspectorServicePath)) {
    const serviceContent = fs.readFileSync(inspectorServicePath, 'utf-8');
    hasControls = serviceContent.includes('generateControlFields') && 
...
function extractNodeSpecification(kind: string, domain: string): NodeDocData['specification'] {
  ...
  if (schemaMatch) {
    ...
    const altSchemaMatch = nodeContent.match(/const\s+\w+DataSchema\s*=\s*z\s*\.\s*object\s*\(\s*{([\s\S]*?)}\s*\)\s*\.\s*passthrough/);
    if (altSchemaMatch) {
      const dataSchema = parseDataSchema(altSchemaMatch[1]);
      return {
        size: {
          expanded: SIZE_MAPPINGS[expandedSize as keyof typeof SIZE_MAPPINGS] || { width: 120, height: 120 },
          collapsed: SIZE_MAPPINGS[collapsedSize as keyof typeof SIZE_MAPPINGS] || { width: 60, height: 60 },
          humanReadable: {
            expanded: SIZE_MAPPINGS[expandedSize as keyof typeof SIZE_MAPPINGS]?.description || "Custom size",
            collapsed: SIZE_MAPPINGS[collapsedSize as keyof typeof SIZE_MAPPINGS]?.description || "Custom size"
          }
        },
        version,
        runtime,
...
function parseMemoryConfig(configStr: string): NodeDocData['specification']['memory'] {
  ...
  return {
    enabled: true,
    maxSize: maxSizeMatch ? parseInt(maxSizeMatch[1]) : 1024 * 1024,
    maxEntries: maxEntriesMatch ? parseInt(maxEntriesMatch[1]) : 1000,
...
function parseDataSchema(schemaStr: string): NodeDocData['specification']['dataSchema'] {
  ...
  lines.forEach(line => {
    ...
    const fieldMatch = line.match(/(\w+):\s*z\.(\w+)(?:\([^)]*\))?(?:\.(\w+)(?:\([^)]*\))?)?(?:\.(\w+)(?:\([^)]*\))?)?/);
    if (fieldMatch) {
 ...
 */
function extractThemingInfo(kind: string, domain: string, category: string): NodeDocData['theming'] {
  const nodeFile = path.join(process.cwd(), 'features/business-logic-modern/node-domain', domain, `${kind}.node.tsx`);
 ...
 */
function extractDesignTokens(category: string, content: string): NodeDocData['theming']['designTokens'] {
  ...
  customTokenPatterns.forEach(({ pattern, property }) => {
    ...
    if (matches && matches.length > 0) {
      tokens[property as keyof typeof tokens] = `var(--node-${matches[0].split('-')[2]}-${property === 'textSecondary' ? 'text-secondary' : property})`;
    }
  });
...
function analyzeAccessibility(content: string): NodeDocData['theming']['accessibility'] {
  const ariaLabels = (content.match(/aria-label="([^"]+)"/g) || [])
    .map(label => label.replace('aria-label="', '').replace('"', ''));
  ...
  const hasScreenReaderSupport = ariaLabels.length > 0 || 
...
function generateMarkdownDoc(nodeData: NodeDocData) {
  const { kind, domain, category, displayName, description, inputs, outputs, examples, relatedNodes } = nodeData;
...
function generateAPIDoc(nodeData: NodeDocData) {
  ...
  const schemaFields = spec.dataSchema?.fields.map(field => {
    const zodType = field.type === 'string' ? 'string' : 
  ...
  const handleSpecs = [...inputs, ...outputs].map(handle => {
    // Map handle types to proper codes based on actual node specification
    let code = 'j'; // default to JSON
    if (handle.type.includes('string') || handle.type.includes('text') || handle.type === 's') code = 's';
    else if (handle.type.includes('number') || handle.type.includes('num') || handle.type === 'n') code = 'n';
    else if (handle.type.includes('boolean') || handle.type.includes('bool') || handle.type === 'b') code = 'b';
  ...
  const initialDataFields = spec.dataSchema?.fields.map(field => 
 ...
 */
export function generateNodeDocs(kind: string, domain: string, category: string, displayName: string) {
