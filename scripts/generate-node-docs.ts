...
const SIZE_MAPPINGS = {
  ...
  C3: {
    width: 180,
    height: 180,
    description: "180×180px (Extra large collapsed)",
  },
  ...
  FE3: {
    width: 240,
    height: 240,
    description: "240×240px (Extra large expanded)",
  },
  ...
  VE0: {
    width: 60,
    height: "auto",
    description: "60px width, variable height",
  },
  VE1: {
    width: 120,
    height: "auto",
    description: "120px width, variable height",
  },
  VE2: {
    width: 180,
    height: "auto",
    description: "180px width, variable height",
  },
  VE3: {
    width: 240,
    height: "auto",
    description: "240px width, variable height",
  },
} as const;
 ...
 */
function detectInspectorIntegration(
  kind: string
): NodeDocData["infrastructure"]["inspector"] {
 ...
 */
function extractNodeSpecification(
  kind: string,
  domain: string
): NodeDocData["specification"] {
  ...
  const sizeMatch = nodeContent.match(
    /size:\s*{\s*expanded:\s*(\w+),\s*collapsed:\s*(\w+)/
  );
  ...
  const runtimeMatch = nodeContent.match(
    /runtime:\s*{\s*execute:\s*["']([^"']+)["']/
  );
  ...
  const controls = controlsMatch
    ? parseControlsConfig(controlsMatch[1])
    : undefined;
  ...
  if (schemaMatch) {
    ...
    if (altSchemaMatch) {
      ...
      return {
        size: {
          expanded: SIZE_MAPPINGS[
            expandedSize as keyof typeof SIZE_MAPPINGS
          ] || {
            width: 120,
            height: 120,
          },
          collapsed: SIZE_MAPPINGS[
            collapsedSize as keyof typeof SIZE_MAPPINGS
          ] || {
            width: 60,
            height: 60,
          },
          humanReadable: {
            expanded:
              SIZE_MAPPINGS[expandedSize as keyof typeof SIZE_MAPPINGS]
                ?.description || "Custom size",
            collapsed:
              SIZE_MAPPINGS[collapsedSize as keyof typeof SIZE_MAPPINGS]
                ?.description || "Custom size",
          },
        },
        version,
...
function parseMemoryConfig(
  configStr: string
): NodeDocData["specification"]["memory"] {
 ...
 */
function parseControlsConfig(
  configStr: string
): NodeDocData["specification"]["controls"] {
  ...
  return {
    ...
    excludeFields: excludeFieldsMatch
      ...
      : [],
    customFields: customFieldsMatch
      ? parseCustomFields(customFieldsMatch[1])
      : [],
  };
}
...
function parseDataSchema(
  schemaStr: string
): NodeDocData["specification"]["dataSchema"] {
  const fields: Array<{
    name: string;
    type: string;
    required: boolean;
...
function analyzeResponsiveDesign(
  content: string
): NodeDocData["theming"]["responsive"] {
...
function analyzeAccessibility(
  content: string
): NodeDocData["theming"]["accessibility"] {
  const ariaLabels = (content.match(/aria-label="([^"]+)"/g) || []).map(
    (label) => label.replace('aria-label="', "").replace('"', "")
  );
 ...
 * Analyze visual states
 */
function analyzeVisualStates(
  content: string
): NodeDocData["theming"]["visualStates"] {
  return {
...
export function generateNodeDocumentation(nodeData: NodeDocData) {
  ...
  nodeData.specification = extractNodeSpecification(
    nodeData.kind,
    nodeData.domain
  );

  // Extract theming information
  nodeData.theming = extractThemingInfo(
    nodeData.kind,
    nodeData.domain,
    nodeData.category
  );
...
### output
${output.map((output) => `- **${output.id}** (${output.type}) - ${output.description} [${output.position}]`).join("\n")}
...
function generateHTMLDoc(nodeData: NodeDocData) {
  const { kind, domain, displayName, description, inputs, output, examples } =
    nodeData;
  const infrastructure = nodeData.infrastructure;
  const spec = nodeData.specification;
...
function generateAPIDoc(nodeData: NodeDocData) {
  const { kind, displayName, inputs, output, category } = nodeData;
  const infrastructure = nodeData.infrastructure;
  const spec = nodeData.specification;
...
`;
  ...
  if (!indexContent.includes(`- [${displayName}](./${domain}/${kind}.md)`)) {
    ...
    if (indexContent.includes("## Available Nodes")) {
      ...
      const availableNodesIndex = lines.findIndex(
        (line) => line === "## Available Nodes"
      );
...
export function generateNodeDocs(
  ...
  const { inputs, output } = extractHandlesFromNodeFile(kind, domain);
...
if (require.main === module) {
  ...
  if (args.length < 4) {
    console.error(
      "Usage: node generate-node-docs.ts <kind> <domain> <category> <displayName>"
    );
    process.exit(1);
  }
