 ...
 */
function extractNodeInfo(content: string, kind: string, domain: string, filePath: string): NodeInfo {
  ...
  const description = descriptionMatch ? 
    descriptionMatch[0].replace(/\/\*\*|\*\//g, '').replace(/\*/g, '').trim() : 
...
function extractThemingInfo(content: string, category: string) {
  ...
  const responsiveClasses = ['sm:', 'md:', 'lg:', 'xl:', '2xl:', 'max-sm:', 'max-md:', 'max-lg:', 'max-xl:'];
  ...
  const accessibility = {
    keyboardSupport: content.includes('onKeyDown') || content.includes('onKeyUp') || content.includes('onKeyPress') || content.includes('tabIndex'),
    screenReaderSupport: content.includes('aria-label') || content.includes('role=') || content.includes('aria-'),
    focusManagement: content.includes('focus') || content.includes('blur') || content.includes('tabIndex')
  };
...
function generateNodesOverview() {
  ...
  console.log(`✅ Found ${domainInfo.reduce((sum, domain) => sum + domain.totalNodes, 0)} nodes across ${domainInfo.length} domains`);
