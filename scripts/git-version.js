...
class GitVersionDetector {
  ...
  getGitInfo() {
    try {
      ...
      const commitShort = this.execGitCommand(
        "git rev-parse --short HEAD"
      ).trim();
      const commitMessage = this.execGitCommand(
        "git log -1 --pretty=%B"
      ).trim();
      const branch = this.execGitCommand(
  ...
  async detectChanges() {
    try {
      ...
      const gitChanged =
        gitInfo.available && lastGitHash && lastGitHash !== gitInfo.hash;
...
`;
      ...
      if (gitInfo.available) {
        console.log(
          `📝 Git info: ${gitInfo.shortHash} on ${gitInfo.branch} by ${gitInfo.author}`
        );
      }
    } catch (error) {
  ...
  showStatus() {
    ...
    try {
      ...
      console.log(
        `📁 Files Tracked: ${cache.hashes ? cache.hashes.length : 0}`
      );
      ...
      if (cache.gitInfo && cache.gitInfo.available) {
        ...
        console.log(
          `   Date: ${new Date(cache.gitInfo.date).toLocaleString()}`
        );
      }
    ...
    case "check":
      ...
      if (changes) {
        ...
        if (changes.gitInfo && changes.gitInfo.available) {
          console.log(
            `📝 Git: ${changes.gitInfo.shortHash} on ${changes.gitInfo.branch}`
          );
        }
      } else {
