  ...
  return new Promise((resolve) => {
    ...
    const req = https.request(requestOptions, (res) => {
      ...
      res.on('end', () => {
        ...
        const hasUtilitekContent = data.includes('Utilitek Solutions') || data.includes('Our expertise is your success');
        console.log(`🏠 Site content: ${hasUtilitekContent ? 'Utilitek homepage' : 'Other/blocked'}`);
        
        // Check for challenge indicators
        const isChallenge = data.includes('challenge') || data.includes('proof-of-work') || data.includes('verification');
        if (isChallenge) {
...
async function test() {
  ...
  const tests = [
    { ua: 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/120.0.0.0 Safari/537.36', desc: 'Real Browser' },
