 ...
 */
function extractParameters(script: string): Array<{name: string; type: string; required: boolean; description?: string; defaultValue?: string}> {
  const parameters: Array<{name: string; type: string; required: boolean; description?: string; defaultValue?: string}> = [];
...
function extractPlopDescription(name: string, plopContent: string): string {
  const pattern = new RegExp(`plop\\.setGenerator\\("${name}",\\s*{[\\s\\S]*?description:\\s*"([^"]+)"`, "g");
 ...
 */
function extractPlopPrompts(name: string, plopContent: string): Array<{name: string; type: string; message: string; optional: boolean; default?: string}> {
  const prompts: Array<{name: string; type: string; message: string; optional: boolean; default?: string}> = [];
  
  const generatorPattern = new RegExp(`plop\\.setGenerator\\("${name}",\\s*{[\\s\\S]*?prompts:\\s*\\[([\\s\\S]*?)\\][\\s\\S]*?}`, "g");
  const match = generatorPattern.exec(plopContent);
  
  if (match) {
    const promptsSection = match[1];
    const promptMatches = promptsSection.match(/{[^}]*name:\s*"([^"]+)"[^}]*type:\s*"([^"]+)"[^}]*message:\s*"([^"]+)"[^}]*}/g);
...
function extractPlopActions(name: string, plopContent: string): Array<{output: string}> {
  ...
  const generatorPattern = new RegExp(`plop\\.setGenerator\\("${name}",\\s*{[\\s\\S]*?actions:\\s*\\[([\\s\\S]*?)\\][\\s\\S]*?}`, "g");
  const match = generatorPattern.exec(plopContent);
...
${analysis.statistics.categories.map(category => `
### ${category}
...
${analysis.packageScripts.concat(analysis.plopGenerators).concat(analysis.customScripts).map(cmd => `
### ${cmd.name}
        ...
        <div class="content">
            ...
            <div class="section">
                ...
                <div id="category-${category}" class="category-section" style="display: none;">
                    ...
                    ${analysis.categories[category]?.map(cmd => `
