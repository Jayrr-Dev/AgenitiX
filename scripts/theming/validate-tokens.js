...
const CONTRAST_REQUIREMENTS = {
  ...
  'node-create-text': { against: 'node-create-bg', ratio: WCAG_AA_NORMAL_RATIO, critical: true },
  'node-create-text-secondary': { against: 'node-create-bg', ratio: WCAG_AA_NORMAL_RATIO, critical: true },
  'node-view-text': { against: 'node-view-bg', ratio: WCAG_AA_NORMAL_RATIO, critical: true },
  'node-view-text-secondary': { against: 'node-view-bg', ratio: WCAG_AA_NORMAL_RATIO, critical: true },
  'node-trigger-text': { against: 'node-trigger-bg', ratio: WCAG_AA_NORMAL_RATIO, critical: true },
  'node-trigger-text-secondary': { against: 'node-trigger-bg', ratio: WCAG_AA_NORMAL_RATIO, critical: true },
  'node-test-text': { against: 'node-test-bg', ratio: WCAG_AA_NORMAL_RATIO, critical: true },
  'node-test-text-secondary': { against: 'node-test-bg', ratio: WCAG_AA_NORMAL_RATIO, critical: true },

  // Infrastructure component tokens
  'infra-inspector-text': { against: 'infra-inspector-bg', ratio: WCAG_AA_NORMAL_RATIO, critical: true },
  'infra-inspector-text-secondary': { against: 'infra-inspector-bg', ratio: WCAG_AA_NORMAL_RATIO, critical: true },
  'infra-sidebar-text': { against: 'infra-sidebar-bg', ratio: WCAG_AA_NORMAL_RATIO, critical: true },
  'infra-sidebar-text-secondary': { against: 'infra-sidebar-bg', ratio: WCAG_AA_NORMAL_RATIO, critical: true },
  'infra-minimap-text': { against: 'infra-minimap-bg', ratio: WCAG_AA_NORMAL_RATIO, critical: true },
  'infra-history-text': { against: 'infra-history-bg', ratio: WCAG_AA_NORMAL_RATIO, critical: true },
  'infra-toolbar-text': { against: 'infra-toolbar-bg', ratio: WCAG_AA_NORMAL_RATIO, critical: true },
  'infra-panel-text': { against: 'infra-panel-bg', ratio: WCAG_AA_NORMAL_RATIO, critical: true },

  // Enhanced contrast checks for hover states (using base text colors)
  'node-create-text-on-hover': { against: 'node-create-bg-hover', ratio: WCAG_AA_NORMAL_RATIO, critical: false, actualToken: 'node-create-text' },
  'node-view-text-on-hover': { against: 'node-view-bg-hover', ratio: WCAG_AA_NORMAL_RATIO, critical: false, actualToken: 'node-view-text' },
  'node-trigger-text-on-hover': { against: 'node-trigger-bg-hover', ratio: WCAG_AA_NORMAL_RATIO, critical: false, actualToken: 'node-trigger-text' },
  'node-test-text-on-hover': { against: 'node-test-bg-hover', ratio: WCAG_AA_NORMAL_RATIO, critical: false, actualToken: 'node-test-text' },
};
...
function parseHSL(hslString, tokens = {}) {
  ...
  const match = resolvedString.match(/^(\d+(?:\.\d+)?)\s+(\d+(?:\.\d+)?)%\s+(\d+(?:\.\d+)?)%$/);
  
  if (!match) {
    console.warn(`Could not parse HSL value: ${hslString} (resolved: ${resolvedString})`);
    return null;
  }
...
function validateTokenContrast(tokens, tokenName, requirement) {
  ...
  if (!tokenValue) {
    return { isValid: false, actualRatio: 0, error: `Token '${actualTokenName}' not found` };
  }

  if (!againstValue) {
    return { isValid: false, actualRatio: 0, error: `Reference token '${requirement.against}' not found` };
  }
  ...
  if (!tokenHSL || !againstHSL) {
    return { isValid: false, actualRatio: 0, error: `Could not parse HSL values for ${tokenName} or ${requirement.against}` };
  }
...
async function validateContrast() {
  ...
  try {
    ...
    console.log(`📋 Extracted ${Object.keys(tokens).length} tokens from globals.css`);
    ...
    for (const [tokenName, requirement] of Object.entries(CONTRAST_REQUIREMENTS)) {
      ...
      if (validation.isValid) {
        console.log(`✅ ${tokenName} vs ${requirement.against}: ${ratioText} (required: ${requiredText})`);
        result.stats.passed++;
      } else {
...
module.exports = { validateContrast, validateTokenContrast, extractTokensFromCSS }; 