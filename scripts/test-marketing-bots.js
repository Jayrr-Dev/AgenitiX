    ...
    const req = https.request({
      ...
      res.on('end', () => {
        ...
        const matches = (
          ...
          (expectedResult === 'BLOCK' && (result === '🚫 BLOCKED' || result === '🚦 RATE LIMITED'))
        );
...
async function runComprehensiveTest() {
  ...
  console.log('Testing both MARKETING BOTS (should be allowed) and MALICIOUS BOTS (should be blocked)\n');
  
  // MARKETING BOTS (Should be ALLOWED)
  const marketingBots = [
    { name: 'Google Search', agent: 'Googlebot/2.1 (+http://www.google.com/bot.html)', expected: 'ALLOW' },
    { name: 'Bing Search', agent: 'Mozilla/5.0 (compatible; Bingbot/2.0; +http://www.bing.com/bingbot.htm)', expected: 'ALLOW' },
    { name: 'Facebook Link Preview', agent: 'facebookexternalhit/1.1 (+http://www.facebook.com/externalhit_uatext.php)', expected: 'ALLOW' },
    { name: 'Twitter Card Bot', agent: 'Twitterbot/1.0', expected: 'ALLOW' },
    { name: 'LinkedIn Bot', agent: 'LinkedInBot/1.0 (compatible; Mozilla/5.0; Apache-HttpClient +http://www.linkedin.com)', expected: 'ALLOW' },
    ...
    { name: 'Google Analytics', agent: 'Mozilla/5.0 (compatible; Google Analytics)', expected: 'ALLOW' },
    { name: 'HubSpot Crawler', agent: 'HubSpot Crawler (+https://www.hubspot.com/)', expected: 'ALLOW' },
    { name: 'Pinterest Bot', agent: 'Pinterest/0.2 (+https://www.pinterest.com/bot.html)', expected: 'ALLOW' },
    { name: 'Discord Bot', agent: 'Mozilla/5.0 (compatible; Discordbot/2.0; +https://discordapp.com)', expected: 'ALLOW' }
  ];
  ...
  const maliciousBots = [
    ...
    { name: 'Headless Chrome', agent: 'Mozilla/5.0 (X11; Linux x86_64) AppleWebKit/537.36 HeadlessChrome/120.0.0.0', expected: 'BLOCK' },
    { name: 'Selenium Bot', agent: 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/120.0.0.0 Safari/537.36 Selenium/4.0', expected: 'BLOCK' },
  ...
  console.log(`🔴 MALICIOUS BOTS: ${maliciousBlocked}/${maliciousTotal} correctly blocked (${((maliciousBlocked/maliciousTotal)*100).toFixed(1)}%)`);
  
  const overallScore = ((marketingAllowed + maliciousBlocked) / (marketingTotal + maliciousTotal)) * 100;
