...
import { RISK_LEVELS, ADAPTIVE_CONFIGS } from '@/lib/anubis/risk-engine';
      ...
      <div className="p-4 border-b border-border">
        ...
        <div className="space-y-2 text-xs">
          <div className="flex justify-between">
            ...
            <span className={`font-medium ${currentConfig.optimisticEnabled ? 'text-green-500' : 'text-red-500'}`}>
          ...
          <div className="flex justify-between">
            ...
            <span className="text-foreground font-medium">
              {currentConfig.gracePeriod / 1000}s
            </span>
          </div>
          ...
          <div className="flex justify-between">
            ...
            <span className={`font-medium ${currentConfig.requiresInteraction ? 'text-yellow-500' : 'text-green-500'}`}>
          ...
          <div className="flex justify-between">
            ...
            <span className={`font-medium ${currentConfig.blockingMode ? 'text-red-500' : 'text-green-500'}`}>
      ...
      <div className="p-4 border-b border-border">
        ...
        <div className="space-y-2 text-xs">
          <div className="flex justify-between">
            ...
            <span className="text-foreground font-medium">{metrics.totalRequests.toLocaleString()}</span>
          </div>
          
          <div className="flex justify-between">
            <span className="text-muted-foreground">Blocked Requests:</span>
            <span className="text-red-500 font-medium">{metrics.blockedRequests.toLocaleString()}</span>
          </div>
          
          <div className="flex justify-between">
            <span className="text-muted-foreground">Optimistic Sessions:</span>
            <span className="text-blue-500 font-medium">{metrics.optimisticSessions.toLocaleString()}</span>
          </div>
          ...
          <div className="flex justify-between">
            ...
            <span className={`font-medium ${
              metrics.threatTrend === 'increasing' ? 'text-red-500' :
              metrics.threatTrend === 'decreasing' ? 'text-green-500' : 'text-blue-500'
            }`}>
              {metrics.threatTrend === 'increasing' ? '↗️ Rising' :
               metrics.threatTrend === 'decreasing' ? '↘️ Falling' : '→ Stable'}
            </span>
          </div>
        </div>
      </div>
