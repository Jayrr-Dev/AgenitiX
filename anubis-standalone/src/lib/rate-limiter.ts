...
export class RateLimiter {
  ...
  checkLimit(request: any): RateLimitResult {
    ...
    console.log(`🚦 Rate limit check for ${key}: ${totalHits}/${this.config.maxRequests} (${allowed ? 'ALLOWED' : 'BLOCKED'})`);
...
export class AdaptiveRateLimiter {
  ...
  constructor() {
    ...
    this.limiters.set('LOW', new RateLimiter({
      windowMs: 15 * 60 * 1000,  // 15 minutes
      maxRequests: 200,          // Generous for trusted users
    }));

    this.limiters.set('MODERATE', new RateLimiter({
      windowMs: 15 * 60 * 1000,  // 15 minutes
      maxRequests: 100,          // Standard limit
    }));

    this.limiters.set('ELEVATED', new RateLimiter({
      windowMs: 15 * 60 * 1000,  // 15 minutes
      maxRequests: 50,           // Reduced for suspicious users
    }));

    this.limiters.set('HIGH', new RateLimiter({
      windowMs: 5 * 60 * 1000,   // 5 minutes
      maxRequests: 10,           // Very restrictive
    }));

    this.limiters.set('DANGEROUS', new RateLimiter({
