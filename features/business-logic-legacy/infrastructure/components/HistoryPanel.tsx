...
import { Clock, RotateCcw, RotateCw, Trash2, ChevronDown, ChevronRight } from 'lucide-react'
...
const HistoryPanel: React.FC<HistoryPanelProps> = ({
  ...
  const getActionIcon = (type: string): React.ReactNode => {
    ...
    return iconMap[type] || <div className="w-2 h-2 bg-gray-500 rounded-full" />
  }
  ...
  const handleJumpToState = (index: number) => {
    if (onJumpToState) {
      ...
      const targetIndex = history.findIndex(entry => entry === visibleHistory[index])
      if (targetIndex !== -1) {
