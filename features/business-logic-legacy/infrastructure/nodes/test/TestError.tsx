...
const BaseActivateResetButton = ({ 
  ...
  const isActive = (data.isGeneratingError === true) || (data.isManuallyActivated === true);
  ...
  const handleReset = (e: React.MouseEvent) => {
    ...
    if (onReset) {
      console.log(`🔗 Error Generator ${id}: Resetting connected nodes immediately...`);
      onReset();
    }
...
// Button for use within node context (can access connections)
const NodeContextActivateResetButton = ({ 
  ...
  const handleConnectedNodesReset = () => {
    ...
    if (targetNodesData.length > 0) {
      console.log(`🔗 Error Generator ${id}: Found ${targetNodesData.length} connected nodes to check for reset`);
      
      targetNodesData.forEach(node => {
        if (node.data?.isErrorState) {
          console.log(`🧹 Error Generator ${id}: Resetting error state on node ${node.id}`, {
            before: { 
          ...
          console.log(`⭕ Error Generator ${id}: Node ${node.id} has no error state to reset`);
        }
      });
    } else {
      console.log(`🔗 Error Generator ${id}: No connected nodes found to reset`);
    }
  };
...
const TestError = createNodeComponent<TestErrorData>(
  createTriggeredNodeConfig<TestErrorData>({
    nodeType: 'testError',
    category: 'test', // Orange/red theme for test nodes
    ...
    // Standard icon node size (square when collapsed)
    size: {
      collapsed: {
        width: 'w-[120px]',
        height: 'h-[120px]'
    ...
    // Define handles (trigger input added automatically, plus JSON output for Vibe Mode)
    handles: [
      { id: 'j', dataType: 'j', position: Position.Bottom, type: 'source' }
    ],
    
    // Enhanced processing logic with trigger and error generation
    processLogic: ({ data, connections, nodesData, updateNodeData, id, setError }) => {
      try {
        ...
        if (data.isManuallyActivated === false && data.isGeneratingError === false && 
            (!data.text || data.text === '') && (!data.json || data.json === '')) {
          setError(null); // Clear any lingering system errors
          console.log(`🧹 Error Generator ${id}: Detected complete reset - ensuring clean state`);
        ...
        if (data.isManuallyActivated) {
          ...
          switch (data.triggerMode) {
            case 'always':
              ...
              break;
            case 'trigger_on':
              shouldGenerateError = triggerConnections.length === 0 || isTriggerActive;
              activationReason = triggerConnections.length === 0 ? 'no trigger connected' : `trigger ${isTriggerActive ? 'ON' : 'OFF'}`;
              break;
            case 'trigger_off':
              shouldGenerateError = triggerConnections.length > 0 && !isTriggerActive;
        ...
        if (shouldGenerateError) {
          ...
          console.log(`⚡ Error Generator ${id}: GENERATING ${errorType.toUpperCase()} error (${activationReason})`);
          console.log(`📄 Error Generator ${id}: Message: "${errorMsg}"`);
          
          // Create actual console errors for testing
          switch (errorType) {
            case 'warning':
              console.warn(`⚠️ TEST WARNING from Error Generator ${id}: ${errorMsg}`);
              break;
            case 'error':
              console.error(`🚨 TEST ERROR from Error Generator ${id}: ${errorMsg}`);
              break;
            case 'critical':
              console.error(`🔥 TEST CRITICAL ERROR from Error Generator ${id}: ${errorMsg}`);
          ...
          console.log(`🔮 Error Generator ${id}: Vibe Mode JSON output:`, errorJson);
          ...
          if (errorType === 'critical') {
            ...
            console.log(`🟡 Error Generator ${id}: Generated WARNING (no system error)`);
          }
          ...
          console.log(`💤 Error Generator ${id}: Not generating error (${activationReason})`);
          setError(null);
        ...
        console.error(`❌ Error Generator ${id}: Processing error:`, processingError);
    ...
    // Collapsed state rendering with error indicators
    renderCollapsed: ({ data, error, updateNodeData, id }) => {
      ...
      const enhancedUpdateNodeData = (nodeId: string, updates: Partial<TestErrorData>) => {
      ...
      return (
        <div className="absolute inset-0 flex flex-col items-center justify-center px-2">
          ...
          {/* Status Display */}
          {!error && (
            <div className={`text-xs text-center font-medium leading-tight mb-2 ${statusColor}`}>
              {data.isGeneratingError ? (
                ...
                <>
                  <div>Ready</div>
                  <div className="text-xs opacity-75">{data.triggerMode || 'trigger_on'}</div>
                </>
              )}
            </div>
          )}
    ...
    // Expanded state rendering with full error configuration
      ...
      return (
        <div className="flex text-xs flex-col w-auto">
          <div className={`font-semibold mb-2 flex items-center justify-between ${categoryTextTheme.primary}`}>
            <span>{error ? 'System Error' : 'Error Generator'}</span>
            {data.isGeneratingError && (
          ...
          <div className="space-y-2">
            ...
            <div>
              ...
              <textarea
                ...
                onChange={(e) => updateNodeData(id, { errorMessage: e.target.value })}
            ...
            <div>
              <label className="block text-xs font-medium mb-1">Error Type:</label>
              <select
                className="w-full text-xs px-2 py-1 rounded border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-800"
                value={errorType}
                onChange={(e) => updateNodeData(id, { errorType: e.target.value as 'warning' | 'error' | 'critical' })}
              >
            ...
            <div>
              <label className="block text-xs font-medium mb-1">Trigger Mode:</label>
              <select
                className="w-full text-xs px-2 py-1 rounded border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-800"
                value={data.triggerMode || 'trigger_on'}
                onChange={(e) => updateNodeData(id, { triggerMode: e.target.value as 'always' | 'trigger_on' | 'trigger_off' })}
              >
            ...
            {data.isGeneratingError && data.json && (
              <div>
                <label className="block text-xs font-medium mb-1">JSON Output (for Vibe Mode):</label>
    ...
    // Inspector controls for detailed error configuration
      ...
      <div className="space-y-4">
        ...
        <div>
          ...
          <select
            ...
            onChange={(e) => updateNodeData(node.id, { errorType: e.target.value })}
          >
            <option value="warning" className="bg-white dark:bg-gray-800 text-gray-900 dark:text-gray-100">Warning</option>
        ...
        <div>
          ...
          <select
            ...
            onChange={(e) => updateNodeData(node.id, { triggerMode: e.target.value })}
          >
            <option value="always" className="bg-white dark:bg-gray-800 text-gray-900 dark:text-gray-100">Always Generate</option>
            <option value="trigger_on" className="bg-white dark:bg-gray-800 text-gray-900 dark:text-gray-100">Generate When Triggered ON</option>
            <option value="trigger_off" className="bg-white dark:bg-gray-800 text-gray-900 dark:text-gray-100">Generate When Triggered OFF</option>
          </select>
        </div>
        ...
        {node.data.isGeneratingError && (
          <div className="p-2 bg-orange-50 dark:bg-orange-900/30 border border-orange-200 dark:border-orange-800 rounded">
            ...
            <div className="text-xs text-orange-600 dark:text-orange-400">
              Type: {node.data.errorType || 'error'}<br/>
              Message: {node.data.errorMessage || 'Custom error message'}
              {node.data.isManuallyActivated && <><br/>Mode: Manually Activated</>}
            </div>
          </div>
        )}
      </div>
