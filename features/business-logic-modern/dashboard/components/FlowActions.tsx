...
export const FlowActions: React.FC<FlowActionsProps> = ({
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      // Clear local IndexedDB caches related to this flow
      try {
        const dbName = "agenitix_history_cache";
        const storeName = "history_snapshots";
        await new Promise<void>((resolve, reject) => {
          ...
          openReq.onsuccess = () => {
            ...
            const key = `flow_${flow.id}_user_${user.id}`;
            store.delete(key);
            tx.oncomplete = () => {
              db.close();
              resolve();
