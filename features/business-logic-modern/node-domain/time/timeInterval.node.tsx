...
import { StatusValueDisplay, ValueDisplay } from "./components/ValueDisplay";
import {
  convertToMs,
  detectDataType,
  formatInterval,
...
export const TimeIntervalDataSchema = z
  .object({
    ...
    outputType: z
      .enum(["counter", "timestamp", "boolean", "custom"])
      .default("counter"),
...
const CONTENT = {
  ...
  runningIndicator:
    "absolute top-1 right-1 w-2 h-2 bg-emerald-400 rounded-full animate-pulse",
...
function createDynamicSpec(data: TimeIntervalData): NodeSpec {
  ...
  switch (data.outputType) {
    ...
    case "custom":
      ...
      if (typeof data.customValue === "number") {
        ...
        typeof data.customValue === "object" &&
  ...
  return {
    ...
    description:
      "Emits pulses at regular intervals with configurable timing and output types",
    feature: "base",
  ...
  ({ id, data, spec }: NodeProps & { spec: NodeSpec }) => {
    ...
    const startInterval = useCallback(() => {
      ...
      if (!currentNodeData.isEnabled || currentNodeData.isRunning) {
        // Cannot start - check enabled and running state
        return;
      }
      ...
      const intervalMs = convertToMs(
        currentConfig.intervalAmount,
        currentConfig.intervalUnit
      );
      const now = Date.now();
      ...
      const firstOutput = generateOutputValue(
        currentConfig.outputType,
        firstCount,
        currentConfig.customValue
      );
      ...
      updateNodeData({
        ...
        output: firstOutput,
        store:
          typeof firstOutput === "object" && firstOutput !== null
      ...
      const intervalId = setInterval(() => {
        ...
        const currentConfig = currentConfigRef.current;
        const outputValue = generateOutputValue(
          currentConfig.outputType,
          currentCount,
          currentConfig.customValue
        );
        const currentTime = Date.now();
        const nextIntervalMs = convertToMs(
          currentConfig.intervalAmount,
          currentConfig.intervalUnit
        );

        console.log(
          "📤 Updating node data - count:",
          currentCount,
          "output:",
          outputValue
        );
    ...
    useEffect(() => {
      if (!isRunning && pulseCount > 0) {
        ...
        const previewValue = generateOutputValue(
          outputType,
          pulseCount,
          customValue
        );
    ...
    useEffect(() => {
      return () => {
        console.log(
          "🧹 CLEANUP ON UNMOUNT - clearing interval:",
          intervalRef.current
        );
        if (intervalRef.current) {
          clearInterval(intervalRef.current);
    ...
    const timeUntilNext = nextPulseTime
      ? Math.max(0, nextPulseTime - Date.now())
      : 0;
    ...
    return (
      <>
        ...
        spec.size.collapsed.height === 60 ? (
          ...
          <LabelNode
            nodeId={id}
            label={(nodeData as TimeIntervalData).label || spec.displayName}
          />
        )}
                  ...
                  {outputType === "custom" && (
                    <div className={CONTENT.formGroup}>
                      ...
                      <div className="text-[7px] text-slate-500 dark:text-slate-400 mt-1">
                        ...
                        <div className="mt-1">
                          Variables: {"{count}"}, {"{timestamp}"}, {"{time}"},{" "}
                          {"{date}"}, {"{iso}"}
                        </div>
                      </div>
                    </div>
