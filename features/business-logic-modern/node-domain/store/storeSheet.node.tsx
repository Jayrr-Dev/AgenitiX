...
function createDynamicSpec(data: StoreSheetData): NodeSpec {
  ...
  return {
    ...
    description:
      "Store JSON data in Google Sheets with real-time sync and connection management",
    feature: "store",
...
const StoreSheetNode = memo(({ id, data }: NodeProps) => {
  ...
  const handleConnect = useCallback(async () => {
    ...
    if (!currentData.connectedAccount || !currentData.accountEmail) {
      ...
      updateNodeData({
        ...
        lastError:
          "No email account connected - please connect an Email Account node",
      });
      return;
    }
    ...
    if (currentData.connectedAccount?.provider !== "gmail") {
      toast.showError(
        "Google Sheets requires a Gmail account. Please connect a Gmail account."
      );
      updateNodeData({
        connectionStatus: "error",
    ...
    try {
      ...
      if (result.success && result.spreadsheetInfo) {
        ...
        toast.showSuccess(
          `Connected to "${result.spreadsheetInfo.title}" using ${currentData.accountEmail}`
        );
      } else {
        updateNodeData({
          connectionStatus: "error",
        ...
        if (
          ...
          toast.showError(
            "Gmail account needs Google Sheets permissions. Please reconnect your Gmail account and grant Google Sheets access."
          );
        } else {
  ...
  }, [spreadsheetId, typedNodeData, testConnection, updateNodeData, toast]);

  /** Sync data to Google Sheets */
  const handleSync = useCallback(async () => {
    if (!inputData) {
      toast.showError(
        "No data to sync. Connect a data source or use 'Send Test Data'"
      );
      return;
    }
    ...
    try {
      ...
      if (result.success) {
        ...
        toast.showSuccess(
          `Synced ${result.rowsAffected} row(s) to ${currentData.spreadsheetTitle || "Google Sheets"}`
        );
      } else {
        updateNodeData({
  ...
  const computeInputData = useCallback((): any => {
    const dataInputEdge = edges.find(
      (e) =>
        e.target === id &&
        normalizeHandleId(e.targetHandle || "") === "data-input"
    );
    ...
    const outputData =
      src.data?.output ?? src.data?.store ?? src.data?.data ?? src.data;
  ...
  const computeAccountData = useCallback((): any => {
    ...
    const accountInputEdge = edges.find((e) => {
      ...
      return (
        isTargetingUs &&
        (normalizedHandle === "account" || targetHandle === "account-input")
      );
    });
  ...
  const handleSendTestData = useCallback(async () => {
    ...
    try {
      ...
      if (syncResult.success) {
        ...
        toast.showSuccess(
          `✅ Synced ${syncResult.rowsAffected} row(s) to Google Sheets!`
        );
      } else {
        updateNodeData({
      ...
      updateNodeData({
        connectionStatus: "error",
        lastError:
          error instanceof Error ? error.message : "Failed to send test data",
        syncOutput: false,
      });
  ...
  useEffect(() => {
    ...
    if (
      JSON.stringify(newInputData) !== JSON.stringify(currentData.inputData)
    ) {
    ...
    if (
      ...
      JSON.stringify(currentData.connectedAccount)
    ) {
  ...
  useEffect(() => {
    ...
    const isConnectedAndReady =
      connectionStatus === "connected" && isEnabled && isActive;
    const currentData = nodeData as StoreSheetData;
          ...
          <div className="flex flex-col items-center justify-center w-full h-full p-2">
            ...
            <div
              className={`${CONTENT.collapsedStatus} ${
                connectionStatus === "connected"
                  ? CONTENT.collapsedConnected
                  : connectionStatus === "connecting"
                    ? CONTENT.collapsedConnecting
                    : connectionStatus === "error"
                      ? CONTENT.collapsedError
          ...
          <div className={CONTENT.body}>
                ...
                <div className={CONTENT.formGroup}>
                  <div className={CONTENT.formRow}>
                    ...
                    <input
                      ...
                      value={spreadsheetId}
                      onChange={(e) =>
                        handleSpreadsheetIdChange(e.target.value)
                      }
            ...
            <div className={CONTENT.statusSection}>
              <div className="flex flex-col gap-1">
                <div className="flex items-center justify-between">
                  <span className="text-[8px] font-medium text-teal-600 dark:text-teal-400">
                    Status:
                  </span>
                ...
                {typedNodeData.lastError && (
                  <div className="mt-1 p-1 bg-red-100 dark:bg-red-900/30 rounded text-[7px] font-mono">
                    <div className="text-red-600 dark:text-red-400 font-semibold mb-0.5">
                      Error:
                    </div>
            ...
            <div className="flex gap-1 mt-2">
              <button
                ...
                onClick={
                  connectionStatus === "connected" ? handleSync : handleConnect
                }
                disabled={!isEnabled}
              >
