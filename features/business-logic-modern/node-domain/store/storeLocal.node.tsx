...
const SecurityValidation = {
	isProtectedKey: (key: string): boolean => {
		...
		return PROTECTED_KEYS.some((protectedKey) => keyLower.includes(protectedKey.toLowerCase()));
	},
...
const SecurityUtils = {
	...
	getSecurityStatus: (): {
		...
		const rateLimitedNodes = Array.from(nodeRateLimits.entries()).filter(
			([, limit]) =>
				now < limit.resetTime && limit.count >= SECURITY_LIMITS.MAX_OPERATIONS_PER_MINUTE
		).length;
...
const validateNodeData = createNodeValidator(StoreLocalDataSchema, "StoreLocal");
		...
		<div className={`relative w-full ${className}`}>
          ...
          `}
				>
                  ...
                  ${index === currentIndex ? "bg-current opacity-60" : "bg-current opacity-20"}
                `}
							/>
						))}
					</div>
...
const DataPreview: React.FC<DataPreviewProps> = ({
	...
	if (!data || Object.keys(data).length === 0) {
		return <div className="text-xs text-gray-500 italic">No data to {mode}</div>;
	}
	...
	const hasMore =
		maxItems !== Number.POSITIVE_INFINITY && Object.keys(filteredData).length > maxItems;
		...
		<div className={`text-xs space-y-2 w-full ${config.colors.container}`}>
			...
			<div className={`border rounded-md overflow-hidden w-full ${config.colors.tableBorder}`}>
				...
				<div className={`grid grid-cols-2 border-b w-full ${config.colors.tableHeader}`}>
					...
					<div className={`px-2 py-1 font-semibold ${config.colors.headerText} min-w-0`}>Value</div>
				</div>
				...
				{entries.map(([key, value], index) => (
					...
					>
						...
						>
							...
							{(mode === "get" || mode === "delete") && getCurrentLocalStorageValue(key)}
						</div>
					</div>
				))}
			</div>
...
const RetrievedDataDisplay: React.FC<RetrievedDataDisplayProps> = ({
	...
	if (!data || Object.keys(data).length === 0) {
		return <div className="text-xs text-gray-500 italic">No data retrieved</div>;
	}
	...
	const hasMore = maxItems !== Number.POSITIVE_INFINITY && Object.keys(data).length > maxItems;
...
function createDynamicSpec(data: StoreLocalData): NodeSpec {
	const expanded =
		EXPANDED_SIZES[data.expandedSize as keyof typeof EXPANDED_SIZES] ?? EXPANDED_SIZES.FV2;
	const collapsed =
...
const StoreLocalNode = memo(({ id, data, spec }: NodeProps & { spec: NodeSpec }) => {
	// -------------------------------------------------------------------------
			...
			if (src) {
				...
				if (sourceData?.output && typeof sourceData.output === "object") {
					...
					const handleId = dataInputEdge.sourceHandle
						? dataInputEdge.sourceHandle.split("__")[0].split("-")[0].toLowerCase()
						: "output";
			...
			if (src) {
				...
				if (sourceData?.output && typeof sourceData.output === "object") {
					const handleId = triggerInputEdge.sourceHandle
						? triggerInputEdge.sourceHandle.split("__")[0].split("-")[0].toLowerCase()
						: "output";
					const output = sourceData.output as Record<string, any>;
					if (output[handleId] !== undefined) {
				...
				if (triggerInputValue === undefined) {
					triggerInputValue = sourceData?.store ?? sourceData?.data ?? sourceData;
				}
	...
	useNodeDataValidation(StoreLocalDataSchema, "StoreLocal", validation.data, id);
	...
	if (flagState.isLoading) {
		...
		const isSmallNode =
			!isExpanded && (nodeData.collapsedSize === "C1" || nodeData.collapsedSize === "C1W");
		...
		<>
            ...
            `}
				>
					...
					>
						<DataPreview data={inputData} mode={mode} maxItems={Number.POSITIVE_INFINITY} />
					</div>
				</div>
			) : (
				<div
...
const StoreLocalNodeWithDynamicSpec = (props: NodeProps) => {
	...
	const dynamicSpec = useMemo(
		...
		[(nodeData as StoreLocalData).expandedSize, (nodeData as StoreLocalData).collapsedSize]
	);

	// Memoise the scaffolded component to keep focus
	const ScaffoldedNode = useMemo(
		() => withNodeScaffold(dynamicSpec, (p) => <StoreLocalNode {...p} spec={dynamicSpec} />),
		[dynamicSpec]
	);
