 ...
 */
export function toBooleanValue(value: any, strictMode = false, customFalsyValues: string[] = []): boolean {
    ...
    if (strictMode) {
        return !FALSY_VALUES.some(falsyValue =>
...
export function toTextValue(
    ...
    try {
        switch (format) {
            ...
            case "default":
            default:
                // toText should convert ALL values to their JSON string representation
                // This ensures primitives get quotes: true -> "true", false -> "false"
 ...
 */
export function getConversionError(value: any, targetType: string): string | null {
    try {
        switch (targetType) {
            case "boolean":
                toBooleanValue(value);
