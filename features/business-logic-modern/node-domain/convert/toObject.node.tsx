  ...
  return {
    ...
    controls: {
      ...
      customFields: [
        ...
        { key: "customKeys", type: "text", label: "Custom Keys", description: "Comma-separated keys for custom strategy" },
        { key: "preserveType", type: "boolean", label: "Preserve Type", description: "Keep objects as-is when possible" },
...
const ToObjectNode = memo(
  ({ id, data, spec }: NodeProps & { spec: NodeSpec }) => {
    ...
    /** Propagate object output ONLY when node is active AND enabled */
    const propagate = useCallback(
      (value: Record<string, any>) => {
        ...
        if (JSON.stringify(out) !== JSON.stringify(lastOutputRef.current)) {
          ...
          updateNodeData({ objectOutput: out, output: out });
        }
      },
    ...
    const computeInput = useCallback((): any => {
      ...
      // Get the raw input value for object conversion
    ...
    useEffect(() => {
      if (inputValue !== undefined && inputValue !== null) {
        try {
          ...
          const parsedCustomKeys = typeof customKeys === 'string' 
            ? customKeys.split(',').map((k: string) => k.trim()).filter((k: string) => k.length > 0)
            : Array.isArray(customKeys) ? customKeys : [];

          const converted = toObjectValue(inputValue, keyStrategy, parsedCustomKeys, preserveType);
          updateNodeData({
    ...
    // Propagate object output when active and enabled
    useEffect(() => {
      if (isActive && isEnabled && !hasError) {
        propagate(objectOutput);
      } else {
        propagate({});
      }
      blockJsonWhenInactive();
    }, [isActive, isEnabled, hasError, objectOutput, propagate, blockJsonWhenInactive]);
    ...
    return (
      <>
        ...
        {!isExpanded &&
          ...
          <LabelNode nodeId={id} label={(nodeData as ToObjectData).label || spec.displayName} />
        )}
        ...
        {!isExpanded ? (
          ...
          <div className={`${CONTENT.expanded} ${!isEnabled ? CONTENT.disabled : ''}`}>
            <div className="space-y-3">
              ...
              {/* Output Display */}
              <div>
                <label className="text-xs font-medium text-muted-foreground">Object Output:</label>
                <div className={`mt-1 p-2 rounded text-xs font-mono ${hasError
                    ? 'bg-red-100 text-red-700 dark:bg-red-900 dark:text-red-300'
                    : 'bg-green-100 text-green-700 dark:bg-green-900 dark:text-green-300'
                  }`}>
                  {hasError ? `Error: ${errorMessage}` : getObjectDisplay(objectOutput)}
                </div>
              </div>
                ...
                <div>
                  ...
                  <div className="mt-1 text-xs text-muted-foreground">
                    ...
                    {keyStrategy === 'custom' && (
                      <>
                        {' • '}
                        Keys: <span className="font-mono">{Array.isArray(customKeys) ? customKeys.join(', ') : customKeys}</span>
                      </>
                    )}
                    <br />
                    Keys: <span className="font-mono">{Object.keys(objectOutput).length}</span>
                    {' • '}
                    Preserve type: <span className="font-mono">{preserveType ? 'yes' : 'no'}</span>
                  </div>
                </div>
              )}
            </div>
