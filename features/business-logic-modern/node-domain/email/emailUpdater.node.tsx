...
const validateNodeData = createNodeValidator(EmailUpdaterDataSchema, "EmailUpdater");
	...
	return {
		...
		controls: {
			...
			customFields: [
				...
				{ key: "processingDelay", type: "number", label: "Processing Delay (ms)" },
				{ key: "operations.markAsRead", type: "boolean", label: "Mark as Read" },
				{ key: "operations.markAsUnread", type: "boolean", label: "Mark as Unread" },
				{ key: "operations.markAsImportant", type: "boolean", label: "Mark as Important" },
...
const EmailUpdaterNode = memo(({ id, spec }: NodeProps & { spec: NodeSpec }) => {
	// -------------------------------------------------------------------------
	...
	const executeUpdate = useCallback(() => {
		...
		const processBatch = () => {
			...
			if (batch.length === 0) {
				...
				updateNodeData({
					status: success ? "success" : failed === emailIds.length ? "error" : "partial",
					progress: {
						total: emailIds.length,
					...
					errors,
					updatedEmailIds: succeeded > 0 ? emailIds.filter(id => !errors.some(e => e.emailId === id)) : [],
	...
	useNodeDataValidation(EmailUpdaterDataSchema, "EmailUpdater", validation.data, id);
	...
	return (
		<div className={`${CONTENT.expanded} ${!isEnabled ? CONTENT.disabled : ""}`}>
			...
			<div className={CONTENT.body}>
				{/* Email Count */}
				<div className="mb-3">
					<div className="text-xs text-gray-600 dark:text-gray-300 mb-1">Selected Emails:</div>
					<div className="text-sm font-medium">
						{emailIds.length > 0 ? `${emailIds.length} emails selected` : "No emails selected"}
					</div>
				</div>

				{/* Update Operations */}
				<div className="mb-3">
					<div className="text-xs text-gray-600 dark:text-gray-300 mb-2">Update Operations:</div>
					...
					<div className="grid grid-cols-2 gap-2 mb-2">
						<label className="flex items-center space-x-2 text-xs">
							<input
								...
								onChange={(e) => toggleOperation("markAsRead", e.target.checked)}
								disabled={!isEnabled}
								className="rounded"
							/>
						...
						<label className="flex items-center space-x-2 text-xs">
							<input
								...
								onChange={(e) => toggleOperation("markAsUnread", e.target.checked)}
								disabled={!isEnabled}
								className="rounded"
							/>
					...
					<div className="grid grid-cols-2 gap-2 mb-2">
						<label className="flex items-center space-x-2 text-xs">
							<input
								...
								onChange={(e) => toggleOperation("markAsImportant", e.target.checked)}
								disabled={!isEnabled}
								className="rounded"
							/>
						...
						<label className="flex items-center space-x-2 text-xs">
							<input
								...
								onChange={(e) => toggleOperation("markAsNotImportant", e.target.checked)}
								disabled={!isEnabled}
								className="rounded"
							/>
				...
				<div className="mt-auto">
					...
					{status === "partial" && (
						<div className="text-xs text-yellow-600 dark:text-yellow-400 mb-2">
							Updated {progress.succeeded} emails, failed to update {progress.failed} emails
						</div>
					)}
					
					<button
						onClick={executeUpdate}
						disabled={!isEnabled || status === "processing" || emailIds.length === 0}
...
const EmailUpdaterNodeWithDynamicSpec = (props: NodeProps) => {
	...
	const dynamicSpec = useMemo(
		...
		[(nodeData as EmailUpdaterData).expandedSize, (nodeData as EmailUpdaterData).collapsedSize]
	);

	// Memoise the scaffolded component to keep focus
	const ScaffoldedNode = useMemo(
		() => withNodeScaffold(dynamicSpec, (p) => <EmailUpdaterNode {...p} spec={dynamicSpec} />),
		[dynamicSpec]
	);
