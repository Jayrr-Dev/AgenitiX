...
export const EmailReplierDataSchema = z
	.object({
		...
		replyStrategy: z.enum(["auto", "template", "ai-generated", "hybrid"]).default("template"),
		...
		// AI Configuration
		aiModel: z.enum(["gpt-3.5-turbo", "gpt-4", "claude-3", "local"]).default("gpt-3.5-turbo"),
		aiPrompt: z.string().default("Generate a professional and helpful email reply based on the context provided."),
		...
		autoReplyConditions: z.array(z.object({
...
const validateNodeData = createNodeValidator(EmailReplierDataSchema, "EmailReplier");
...
function createDynamicSpec(data: EmailReplierData): NodeSpec {
	...
	return {
		...
		initialData: createSafeInitialData(EmailReplierDataSchema, {
			...
			aiPrompt: "Generate a professional and helpful email reply based on the context provided.",
			includeContext: true,
			maxTokens: 500,
			temperature: 0.7,
			replyToAll: false,
		...
		controls: {
			...
			customFields: [
				...
				{ key: "useCustomTemplate", type: "boolean", label: "Use Custom Template" },
				{
					key: "aiModel",
					type: "select",
					label: "AI Model",
		...
		description: "Automated email reply generation with AI and template support",
		feature: "email",
		tags: ["email", "reply", "automation", "ai", "templates", "smart-responses"],
		theming: {},
	};
}
...
const EmailReplierNode = memo(({ id, spec }: NodeProps & { spec: NodeSpec }) => {
	// -------------------------------------------------------------------------
	...
	const handleStrategyChange = useCallback(
		(e: ChangeEvent<HTMLSelectElement>) => {
			...
			if (newStrategy === "template" && !selectedTemplate && !useCustomTemplate) {
	...
	const handleAIModelChange = useCallback(
		(e: ChangeEvent<HTMLSelectElement>) => {
			updateNodeData({ aiModel: e.target.value as EmailReplierData["aiModel"] });
		},
		[updateNodeData]
	);
	...
	useNodeDataValidation(EmailReplierDataSchema, "EmailReplier", validation.data, id);
	...
	return (
		<>
			{/* Editable label */}
			<LabelNode nodeId={id} label={(nodeData as EmailReplierData).label || spec.displayName} />
			...
			{isExpanded ? (
				<div className={`${CONTENT.expanded} ${isEnabled ? "" : CONTENT.disabled}`}>
					...
					<div className={`${CONTENT.body} max-h-[400px] overflow-y-auto`}>
						...
						<div>
							<label htmlFor="reply-strategy-select" className="mb-1 block text-gray-600 text-xs">
								Reply Strategy:
							</label>
							<select
						...
						{/* AI Configuration */}
						{(replyStrategy === "ai-generated" || replyStrategy === "hybrid") && (
							<div className="space-y-2">
								<div>
									<label htmlFor="ai-model-select" className="mb-1 block text-gray-600 text-xs">
										AI Model:
									</label>
									<select
										id="ai-model-select"
								...
								<div>
									<label htmlFor="ai-prompt-textarea" className="mb-1 block text-gray-600 text-xs">
										AI Prompt:
									</label>
									<textarea
						...
						{addSignature && (
							<div>
								<label htmlFor="signature-textarea" className="mb-1 block text-gray-600 text-xs">
									Signature:
								</label>
								<textarea
						...
						{generatedReply && (
							<div>
								<label className="mb-1 block text-gray-600 text-xs">Generated Reply:</label>
						...
						<div className="rounded bg-gray-50 p-2 text-gray-500 text-xs">
							...
							{lastProcessed && <div>Last: {new Date(lastProcessed).toLocaleString()}</div>}
							{replyMetadata && (
								<div>
									Confidence: {Math.round(replyMetadata.confidence * 100)}% | 
									Time: {replyMetadata.processingTime}ms
									{replyMetadata.tokensUsed && ` | Tokens: ${replyMetadata.tokensUsed}`}
								</div>
							)}
							...
							{isProcessing
								...
								: generatedReply
									...
									: lastError
										? "✗"
										: "○"}{" "}
							{isProcessing ? "Processing" : generatedReply ? "Ready" : lastError ? "Error" : "Idle"}
						</div>
					</div>
				</div>
			)}
...
const EmailReplierNodeWithDynamicSpec = (props: NodeProps) => {
	...
	const dynamicSpec = useMemo(
		...
		[(nodeData as EmailReplierData).expandedSize, (nodeData as EmailReplierData).collapsedSize]
	);

	// Memoise the scaffolded component to keep focus
	const ScaffoldedNode = useMemo(
		() => withNodeScaffold(dynamicSpec, (p) => <EmailReplierNode {...p} spec={dynamicSpec} />),
		[dynamicSpec]
	);
