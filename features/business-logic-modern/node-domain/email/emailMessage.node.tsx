...
function createDynamicSpec(data: EmailMessageData): NodeSpec {
  ...
  return {
    ...
    description: "EmailMessage node for email operations",
    feature: "email",
    tags: ["email", "emailMessage"],
    featureFlag: {
      flag: "test",
      fallback: true,
      disabledMessage: "This emailMessage node is currently disabled",
...
const EmailMessageNode = memo(
  ({ id, data, spec }: NodeProps & { spec: NodeSpec }) => {
    ...
    useEffect(() => {
      ...
      if (inputVal !== (nodeData as EmailMessageData).inputs) {
    ...
    useEffect(() => {
      const incoming = (nodeData as EmailMessageData).inputs;
      if (incoming !== null) {
        const hasValue =
    ...
    return (
      <>
        ...
        spec.size.collapsed.height === 60 ? (
          ...
          <LabelNode nodeId={id} label={(nodeData as EmailMessageData).label || spec.displayName} />
        )}
...
const EmailMessageNodeWithDynamicSpec = (props: NodeProps) => {
  ...
  const dynamicSpec = useMemo(
    () => createDynamicSpec(nodeData as EmailMessageData),
    [
      (nodeData as EmailMessageData).expandedSize,
      (nodeData as EmailMessageData).collapsedSize,
    ],
  );
