  ...
  ({ id, spec }: NodeProps & { spec: NodeSpec }) => {
    ...
    const handleOAuth2Auth = useCallback(async () => {
      ...
      try {
        ...
        const response = await fetch(`/api/auth/email/${provider}?redirect_uri=${encodeURIComponent(redirectUri)}`);
    ...
    const handleAuthSuccess = useCallback(async (authDataEncoded: string | null) => {
    ...
    }, [token, isManualProvider, provider, email, displayName, imapHost, imapPort, smtpHost, smtpPort, username, password, useSSL, useTLS, storeEmailAccount, updateNodeData]);
    ...
    }, [isEnabled, isConnected, provider, email, displayName, nodeData.accountId, lastValidated, updateNodeData]);
          ...
          <div className={`${CONTENT.collapsed} ${!isEnabled ? CONTENT.disabled : ''}`}>
            <div className="text-center p-2">
              ...
              <div className={`text-xs ${connectionStatus === 'connected' ? 'text-green-600' : connectionStatus === 'error' ? 'text-red-600' : 'text-gray-600'}`}>
                {connectionStatus === 'connected' ? '✓' : connectionStatus === 'error' ? '✗' : '○'} {connectionStatus}
              </div>
            </div>
          </div>
        ) : (
