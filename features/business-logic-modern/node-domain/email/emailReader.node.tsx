...
export const EmailReaderDataSchema = z
  .object({
    ...
    connectionStatus: z.enum(["idle", "connecting", "connected", "reading", "processing", "error"]).default("idle"),
...
function createDynamicSpec(data: EmailReaderData): NodeSpec {
  ...
  return {
    ...
    description: "Read and process emails from configured email accounts with filtering and real-time monitoring",
    feature: "email",
        ...
        {!isExpanded ? (
          <div className={`${CONTENT.collapsed} ${!isEnabled ? CONTENT.disabled : ''}`}>
            <div className="text-center p-2">
              ...
              <div className={`text-xs ${connectionStatus === 'connected' ? 'text-green-600' : connectionStatus === 'error' ? 'text-red-600' : 'text-gray-600'}`}>
                {connectionStatus === 'reading' ? '📖' : connectionStatus === 'connected' ? '✓' : connectionStatus === 'error' ? '✗' : '○'} {connectionStatus}
              </div>
            </div>
          </div>
        ) : (
          <div className={`${CONTENT.expanded} ${!isEnabled ? CONTENT.disabled : ''}`}>
            ...
            <div className={`${CONTENT.body} overflow-y-auto max-h-[400px]`}>
              ...
              {enableRealTime && (
                <div>
                  <label className="text-xs text-gray-600 mb-1 block">Check Interval (minutes):</label>
                  <input
                    type="number"
                    value={checkInterval}
              ...
              <div className="text-xs text-gray-500 p-2 bg-gray-50 rounded">
                <div>Messages: {messageCount} | Processed: {processedCount}</div>
                {lastSync && (
