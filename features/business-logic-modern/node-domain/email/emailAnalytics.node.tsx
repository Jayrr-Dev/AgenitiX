...
export const EmailAnalyticsDataSchema = z
  .object({
    ...
    reportType: z
      .enum(["summary", "detailed", "comparison", "trends"])
      .default("summary"),
    ...
    // UI State
    activeTab: z
      .enum(["overview", "trends", "content", "audience"])
      .default("overview"),
  ...
  return {
    ...
    controls: {
      ...
      customFields: [
        ...
        {
          key: "trackingMetrics.clicks",
          type: "boolean",
          label: "Track Clicks",
        },
        {
          key: "trackingMetrics.bounces",
          type: "boolean",
          label: "Track Bounces",
        },
        {
          key: "trackingMetrics.unsubscribes",
          type: "boolean",
          label: "Track Unsubscribes",
        },
        {
          key: "trackingMetrics.conversions",
          type: "boolean",
    ...
    description:
      "Analyze email campaign performance with detailed metrics and reporting",
    feature: "email",
...
const generateMockAnalytics = (
  ...
  const totalOpened = Math.floor(
    totalDelivered * (0.15 + Math.random() * 0.25)
  ); // 15-40% open rate
  ...
  const totalUnsubscribed = Math.floor(
    totalOpened * (0.001 + Math.random() * 0.009)
  ); // 0.1-1% unsub rate
  ...
  const metrics = {
    ...
    unsubscribeRate:
      Math.round((totalUnsubscribed / totalDelivered) * 100 * 100) / 100,
    conversionRate:
      Math.round((Math.floor(totalClicked * 0.1) / totalClicked) * 100 * 100) /
      100,
  };
  ...
  const topLinks = [
    {
      ...
      clicks: Math.floor(totalClicked * 0.3),
      clickRate: 30,
    },
    {
      url: "https://example.com/product2",
      clicks: Math.floor(totalClicked * 0.25),
      clickRate: 25,
    },
    {
      url: "https://example.com/blog",
      clicks: Math.floor(totalClicked * 0.2),
      clickRate: 20,
    },
    {
      url: "https://example.com/contact",
      clicks: Math.floor(totalClicked * 0.15),
      clickRate: 15,
    },
    {
      url: "https://example.com/about",
      clicks: Math.floor(totalClicked * 0.1),
      clickRate: 10,
    },
  ];
  ...
  const geographicData = [
    {
      ...
      opens: Math.floor(totalOpened * 0.4),
      clicks: Math.floor(totalClicked * 0.4),
    },
    {
      country: "United Kingdom",
      opens: Math.floor(totalOpened * 0.15),
      clicks: Math.floor(totalClicked * 0.15),
    },
    {
      country: "Canada",
      opens: Math.floor(totalOpened * 0.12),
      clicks: Math.floor(totalClicked * 0.12),
    },
    {
      country: "Australia",
      opens: Math.floor(totalOpened * 0.1),
      clicks: Math.floor(totalClicked * 0.1),
    },
    {
      country: "Germany",
      opens: Math.floor(totalOpened * 0.08),
      clicks: Math.floor(totalClicked * 0.08),
    },
  ];
    ...
    return (
      ...
      >
        ...
        <div className={CONTENT.header}>
          <LabelNode nodeId={id} label="Email Analytics" />
            ...
            <div className="mb-3">
              ...
              <div className="space-y-1">
                {topLinks.slice(0, 3).map((link, i) => (
                  <div
                    key={i}
                    className="flex justify-between items-center text-xs p-1 bg-gray-50 dark:bg-gray-800 rounded"
                  >
...
const EmailAnalyticsNodeWithDynamicSpec = (props: NodeProps) => {
  ...
  const dynamicSpec = useMemo(
    ...
    [
      (nodeData as EmailAnalyticsData).expandedSize,
      (nodeData as EmailAnalyticsData).collapsedSize,
    ]
  );
  ...
  const ScaffoldedNode = useMemo(
    () =>
      withNodeScaffold(dynamicSpec, (p) => (
        <EmailAnalyticsNode {...p} spec={dynamicSpec} />
      )),
    [dynamicSpec]
  );
