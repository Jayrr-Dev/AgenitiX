...
function createDynamicSpec(data: EmailListData): NodeSpec {
  ...
  return {
    ...
    description:
      "Manage email distribution lists and contacts with import/export capabilities",
    feature: "email",
...
const EmailListNode = memo(({ id, spec }: NodeProps & { spec: NodeSpec }) => {
  ...
  const importEmails = useCallback(() => {
    ...
    updateNodeData({
      isProcessing: true,
      processingStep: "Importing emails...",
    });
        ...
        updateNodeData({
          ...
          errorOutput:
            result.invalid.length > 0
              ? `Found ${result.invalid.length} invalid emails`
              : "",
        });
  ...
  return (
    <div className={`${CONTENT.expanded} ${isEnabled ? "" : CONTENT.disabled}`}>
      ...
      <div className={CONTENT.header}>
        <LabelNode nodeId={id} label="Email List" />
      ...
      <div className={CONTENT.body}>
        ...
        <div className="space-y-2 mb-3">
          ...
          <div>
            ...
            <input
              ...
              onChange={(e) =>
                updateNodeData({ listDescription: e.target.value })
              }
              placeholder="List description"
              className="w-full px-2 py-1 text-xs border border-gray-300 dark:border-gray-600 rounded bg-white dark:bg-gray-800 text-gray-900 dark:text-gray-100"
              disabled={!isEnabled}
        ...
        <div className="flex-1 min-h-0">
          <div className="text-xs font-medium text-gray-700 dark:text-gray-300 mb-2">
            Email List ({emails.length} total, {validEmails.length} valid,{" "}
            ...
            {emails.length === 0 ? (
              ...
              <div className="space-y-1">
                {emails.map((email) => (
                  <div
                    key={email}
                    className="flex items-center justify-between text-xs p-1 hover:bg-gray-100 dark:hover:bg-gray-700 rounded"
                  >
                    <span
                      className={
                        validateEmail(email)
...
const EmailListNodeWithDynamicSpec = (props: NodeProps) => {
  ...
  const dynamicSpec = useMemo(
    ...
    [
      (nodeData as EmailListData).expandedSize,
      (nodeData as EmailListData).collapsedSize,
    ]
  );
  ...
  const ScaffoldedNode = useMemo(
    () =>
      withNodeScaffold(dynamicSpec, (p) => (
        <EmailListNode {...p} spec={dynamicSpec} />
      )),
    [dynamicSpec]
  );
