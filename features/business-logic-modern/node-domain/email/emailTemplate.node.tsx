...
export const EmailTemplateDataSchema = z
	.object({
		...
		variables: z.array(z.object({
			name: z.string(),
			type: z.enum(["text", "number", "date", "boolean", "email", "url"]).default("text"),
...
const validateNodeData = createNodeValidator(EmailTemplateDataSchema, "EmailTemplate");
	...
	return {
		...
		controls: {
			...
			customFields: [
				...
				{ key: "templateName", type: "text", label: "Template Name", placeholder: "Enter template name" },
				{ key: "templateDescription", type: "textarea", label: "Description", placeholder: "Template description" },
				{
					key: "category",
					type: "select",
					label: "Category",
				...
				{ key: "subject", type: "text", label: "Subject", placeholder: "Email subject with {{variables}}" },
				{ key: "htmlContent", type: "textarea", label: "HTML Content", placeholder: "HTML email content" },
				{ key: "textContent", type: "textarea", label: "Text Content", placeholder: "Plain text fallback" },
		...
		description: "Create and manage email templates with dynamic variables and preview functionality",
		feature: "email",
...
const EmailTemplateNode = memo(({ id, spec }: NodeProps & { spec: NodeSpec }) => {
	// -------------------------------------------------------------------------
	...
	const saveTemplateMutation = useMutation(api.emailAccounts.storeEmailReplyTemplate);

	// -------------------------------------------------------------------------
	// 4.4  Available templates for selection
	// -------------------------------------------------------------------------
	const availableTemplates = useMemo(() => {
		if (!(emailTemplates?.success && Array.isArray(emailTemplates.data))) {
			return [];
		}
	...
	const handleSaveTemplate = useCallback(async () => {
		...
		try {
			...
			updateNodeData({
				isSaving: false,
				lastError: error instanceof Error ? error.message : "Failed to save template",
			});
			toast.error("Failed to save template", {
				description: error instanceof Error ? error.message : "Unknown error",
			});
		}
	}, [token, templateName, category, subject, htmlContent, textContent, variables, templateDescription, saveTemplateMutation, updateNodeData]);
	...
	}, [isEnabled, templateName, category, subject, htmlContent, textContent, variables, previewData, templateId, compileTemplate, updateNodeData]);
	...
	useNodeDataValidation(EmailTemplateDataSchema, "EmailTemplate", validation.data, id);
		...
		<div className={`${CONTENT.expanded} ${!isEnabled ? CONTENT.disabled : ""}`}>
			...
			<div className={CONTENT.body}>
				...
				<div className="space-y-2">
					<div>
						<label htmlFor={`template-name-${id}`} className="block text-xs font-medium text-gray-700 dark:text-gray-300 mb-1">
							Template Name
						</label>
						<input
							id={`template-name-${id}`}
					...
					<div>
						<label htmlFor={`template-category-${id}`} className="block text-xs font-medium text-gray-700 dark:text-gray-300 mb-1">
							Category
						</label>
						<select
					...
					<div>
						<label htmlFor={`template-subject-${id}`} className="block text-xs font-medium text-gray-700 dark:text-gray-300 mb-1">
							Subject
						</label>
						<input
				...
				<div className="space-y-2">
					<div>
						<label htmlFor={`template-html-${id}`} className="block text-xs font-medium text-gray-700 dark:text-gray-300 mb-1">
							HTML Content
						</label>
						<textarea
							id={`template-html-${id}`}
							value={htmlContent}
							onChange={(e) => updateNodeData({ htmlContent: e.target.value })}
					...
					<div>
						<label htmlFor={`template-text-${id}`} className="block text-xs font-medium text-gray-700 dark:text-gray-300 mb-1">
							Text Content
						</label>
						<textarea
							id={`template-text-${id}`}
							value={textContent}
							onChange={(e) => updateNodeData({ textContent: e.target.value })}
				...
				{showPreview && templateName && (
					<div className="p-2 bg-gray-50 dark:bg-gray-800 rounded border">
						...
						<div className="text-xs text-gray-600 dark:text-gray-400">
							<div><strong>Subject:</strong> {compileTemplate().subject}</div>
							<div className="mt-1"><strong>Content:</strong></div>
							<div className="mt-1 p-1 bg-white dark:bg-gray-700 rounded text-xs max-h-20 overflow-y-auto">
								{compileTemplate().html || compileTemplate().text || "No content"}
							</div>
						</div>
					</div>
				)}
...
const EmailTemplateNodeWithDynamicSpec = (props: NodeProps) => {
	...
	const dynamicSpec = useMemo(
		...
		[(nodeData as EmailTemplateData).expandedSize, (nodeData as EmailTemplateData).collapsedSize]
	);

	// Memoise the scaffolded component to keep focus
	const ScaffoldedNode = useMemo(
		() => withNodeScaffold(dynamicSpec, (p) => <EmailTemplateNode {...p} spec={dynamicSpec} />),
		[dynamicSpec]
	);
