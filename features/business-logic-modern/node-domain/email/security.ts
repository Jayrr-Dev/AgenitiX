...
export function decryptCredentials(encryptedData: string): EmailAccountConfig {
  try {
    ...
    throw new Error(`Failed to decrypt credentials: ${error instanceof Error ? error.message : 'Invalid encrypted data'}`);
  }
}
...
export function validateCredentials(credentials: EmailAccountConfig): EmailError | null {
  ...
  switch (credentials.provider) {
    ...
    case 'outlook':
      ...
      // Check token expiry
      if (credentials.tokenExpiry && credentials.tokenExpiry < Date.now() + SECURITY_CONFIG.TOKEN_EXPIRY_BUFFER) {
        return createEmailError(
          'TOKEN_EXPIRED',
