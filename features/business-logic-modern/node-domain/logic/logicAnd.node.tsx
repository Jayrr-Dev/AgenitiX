/**
 * logicAnd NODE – Primitive boolean AND gate (60x60px)
 * 
...
export const LogicAndDataSchema = z
	.object({
		// Core logic state
...
export type LogicAndData = z.infer<typeof LogicAndDataSchema>;

// Dynamic spec
function createDynamicSpec(data: LogicAndData): NodeSpec {
	return {
		kind: "logicAnd",
		...
		inspector: { key: "LogicAndInspector" },
		version: 1,
		runtime: { execute: "logicAnd_execute_v1" },
		initialData: createSafeInitialData(LogicAndDataSchema, {
			output: null,
		...
		description: "Boolean AND gate - outputs true when ALL inputs are true",
		feature: "base",
		tags: ["logic", "boolean", "and", "gate", "primitive"],
		theming: {},
	};
}
...
const LogicAndNode = memo(({ id, data, spec }: NodeProps & { spec: NodeSpec }) => {
	// -------------------------------------------------------------------------
	...
	const computeConnectionStates = useCallback(() => {
		...
		// Apply AND logic: all inputs must be true
		const logicResult = LogicOperations.and(values);
	...
	return (
		<div className={`flex flex-col items-center justify-center w-full h-full p-1 ${isEnabled ? "" : "opacity-50"}`}>
			<div className="text-xs font-bold text-muted-foreground mb-1">AND</div>
...
// Wrapper component
const LogicAndNodeWithDynamicSpec = (props: NodeProps) => {
	const { nodeData } = useNodeData(props.id, props.data);
	const dynamicSpec = useMemo(() => createDynamicSpec(nodeData as LogicAndData), [nodeData]);
	const ScaffoldedNode = useMemo(
		() => withNodeScaffold(dynamicSpec, (p) => <LogicAndNode {...p} spec={dynamicSpec} />),
		[dynamicSpec]
	);
	return <ScaffoldedNode {...props} />;
};

export default LogicAndNodeWithDynamicSpec;
