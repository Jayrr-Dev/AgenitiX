/**
 * logicXor NODE – Primitive boolean XOR gate (60x60px)
 * 
...
export const LogicXorDataSchema = z
	.object({
		// Core logic state
...
export type LogicXorData = z.infer<typeof LogicXorDataSchema>;

// Dynamic spec
function createDynamicSpec(data: LogicXorData): NodeSpec {
	return {
		kind: "logicXor",
		...
		inspector: { key: "LogicXorInspector" },
		version: 1,
		runtime: { execute: "logicXor_execute_v1" },
		initialData: createSafeInitialData(LogicXorDataSchema, {
			output: null,
		...
		description: "Boolean XOR gate - outputs true when an ODD number of inputs are true",
		feature: "base",
		tags: ["logic", "boolean", "xor", "gate", "primitive"],
		theming: {},
	};
}
...
const LogicXorNode = memo(({ id, data, spec }: NodeProps & { spec: NodeSpec }) => {
	// -------------------------------------------------------------------------
	...
	const computeConnectionStates = useCallback(() => {
		...
		// Apply XOR logic: odd number of inputs must be true
		const logicResult = LogicOperations.xor(values);
	...
	return (
		<div className={`flex flex-col items-center justify-center w-full h-full p-1 ${isEnabled ? "" : "opacity-50"}`}>
			<div className="text-xs font-bold text-muted-foreground mb-1">XOR</div>
...
// Wrapper component
const LogicXorNodeWithDynamicSpec = (props: NodeProps) => {
	const { nodeData } = useNodeData(props.id, props.data);
	const dynamicSpec = useMemo(() => createDynamicSpec(nodeData as LogicXorData), [nodeData]);
	const ScaffoldedNode = useMemo(
		() => withNodeScaffold(dynamicSpec, (p) => <LogicXorNode {...p} spec={dynamicSpec} />),
		[dynamicSpec]
	);
	return <ScaffoldedNode {...props} />;
};

export default LogicXorNodeWithDynamicSpec;
