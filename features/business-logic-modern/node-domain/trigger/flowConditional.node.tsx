...
export const FlowConditionalNode = memo(
  ({ id, data, spec }: NodeProps & { spec: NodeSpec }) => {
    ...
    // 4.2  Derived state
    // -------------------------------------------------------------------------
    const { isExpanded, isEnabled, isActive, condition, lastRoute } =
    ...
    const categoryStyles = CATEGORY_TEXT.CYCLE;
    ...
    const computeConditionInput = useCallback((): boolean | null => {
      ...
      if (!conditionInputEdge) return null;
      ...
      return conditionValue === true || conditionValue === "true" || conditionValue === 1 || conditionValue === "1";
    }, [edges, nodes, id]);
        ...
        {!isExpanded ? (
          <div 
            ...
            <div className={CONTENT.route}>
              ...
              <div className={`${CONTENT.arrow} ${CONTENT.arrowRight} ${lastRoute === "true" ? CONTENT.arrowActive : ''}`}>
                →
              </div>
              
              {/* Bottom arrow (false path) */}
              <div className={`${CONTENT.arrow} ${CONTENT.arrowBottom} ${lastRoute === "false" ? CONTENT.arrowActive : ''}`}>
                →
              </div>
            </div>
          </div>
      ...
      <div 
        ...
        <div className={CONTENT.body}>
          <div className="flex flex-col items-center justify-center w-full h-full">
            ...
            <div className={`mt-1 font-bold ${lastRoute === "true" ? CONTENT.trueRoute : lastRoute === "false" ? CONTENT.falseRoute : CONTENT.noRoute}`}>
              {lastRoute === "true" ? "TRUE PATH" : lastRoute === "false" ? "FALSE PATH" : "NO ROUTE"}
            </div>
            
            <div className="mt-4 text-xs text-gray-500">
              {(nodeData as FlowConditionalData).conditionInput !== null ? "Using external condition input" : "Using internal condition value"}
            </div>
          </div>
        </div>
      </div>
