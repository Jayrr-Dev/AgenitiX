...
const CreateTextNode = memo(({ id, data, spec }: NodeProps & { spec: NodeSpec }) => {
	...
	const propagate = useCallback(
		(value: string) => {
			const shouldSend =
				(nodeData as CreateTextData).isActive && (nodeData as CreateTextData).isEnabled;
	...
	useEffect(() => {
		...
		(nodeData as CreateTextData).store,
		(nodeData as CreateTextData).isActive,
		(nodeData as CreateTextData).isEnabled,
		updateNodeData,
	]);
			...
			{!isExpanded ? (
				<div className={CONTENT.collapsed}>
					<div className="text-center">
						<div
							className={`text-xs font-medium ${categoryStyles.primary} tracking-wide max-w-20 truncate`}
						>
							<textarea
