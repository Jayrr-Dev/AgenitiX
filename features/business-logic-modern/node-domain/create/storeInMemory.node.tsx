...
function createDynamicSpec(data: StoreInMemoryData): NodeSpec {
  ...
  return {
    ...
    description:
      "Stores data in memory during workflow execution with get/set operations",
    feature: "storage",
  ...
  ({ id, data, spec }: NodeProps & { spec: NodeSpec }) => {
    ...
    return (
      <>
        ...
        <LabelNode
          nodeId={id}
          label={(nodeData as StoreInMemoryData).label || spec.displayName}
        />

        {isExpanded ? (
          <div
            className={`${CONTENT.expanded} ${isEnabled ? "" : "opacity-50 cursor-not-allowed"}`}
          >
          ...
          <div
            className={`${CONTENT.collapsed} ${isEnabled ? "" : "opacity-50 cursor-not-allowed"}`}
          >
