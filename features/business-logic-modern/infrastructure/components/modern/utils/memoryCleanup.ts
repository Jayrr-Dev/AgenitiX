...
const clearNodeFactoryCaches = (): number => {
  ...
  try {
    ...
    const { calculationCache, debouncedUpdates, NODE_INSPECTOR_REGISTRY } = require('../factory/NodeFactory');
...
const clearGlobalEventListeners = (): number => {
  ...
  try {
    ...
    const events = ['resize', 'scroll', 'keydown', 'mousemove', 'touchmove', 'touchend'];
