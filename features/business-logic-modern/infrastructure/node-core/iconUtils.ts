...
const POPULAR_ICONS_CACHE = new Map<
  string,
  React.ComponentType<{ className?: string; size?: number }>
>();
...
const initializePopularIcons = () => {
  ...
  popularIconNames.forEach((iconName) => {
    ...
    if (IconComponent) {
      const MemoizedIcon = memo<{ className?: string; size?: number }>(
        ({ className = "", size = 16 }) =>
...
const getCachedIconComponent = (
  ...
  const cachedComponent = ICON_COMPONENT_CACHE.get(iconName);
  if (cachedComponent) {
...
export function renderLucideIcon(
  ...
  if (!CachedIconComponent) {
    console.warn(
      `⚠️ Icon "${iconName}" not found in react-icons/lu. Using fallback.`
    );
    // Return cached fallback icon
    const FallbackComponent = getCachedIconComponent("LuCircle");
    return FallbackComponent
      ? React.createElement(FallbackComponent, { className, size })
      : null;
  }
...
export const LucideIcon = memo<LucideIconProps>(
  ({ name, className = "", size = 16 }) => {
    ...
    if (!IconComponent) {
      console.warn(
        `⚠️ Icon "${name}" not found in react-icons/lu. Using fallback.`
      );
      const FallbackComponent = getCachedIconComponent("LuCircle");
      return FallbackComponent
        ? React.createElement(FallbackComponent, { className, size })
