  ...
  return function validateNodeData(data: unknown): ValidationResult<T> {
    try {
      ...
      if (error instanceof z.ZodError) {
        ...
        console.error(`${nodeType} node data validation failed:`, errorContext);
...
export const CommonSchemas = {
  ...
  text: z.string()
    ...
    .refine(text => !/<script\b[^<]*(?:(?!<\/script>)<[^<]*)*<\/script>/gi.test(text), 
  ...
  safeHtml: z.string()
    .refine(html => !/<script\b[^<]*(?:(?!<\/script>)<[^<]*)*<\/script>/gi.test(html), 
            'Script tags are not allowed')
    .refine(html => !/<iframe\b[^<]*(?:(?!<\/iframe>)<[^<]*)*<\/iframe>/gi.test(html), 
...
export function reportValidationError(
  ...
  console.error('Node validation error report:', errorReport);
  
  // Report to enterprise error tracking service
  if (errorTrackingService) {
    errorTrackingService.captureError(new Error(`Validation failed for ${nodeType} node`), errorReport);
  }
}
