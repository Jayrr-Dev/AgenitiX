 ...
 */
function updateValidationMetrics(nodeType: string, success: boolean, errors?: z.ZodIssue[]) {
 ...
 */
export function createNodeValidator<T>(schema: z.ZodSchema<T>, nodeType: string) {
		...
		try {
			...
			if (error instanceof z.ZodError) {
				...
				try {
					...
					console.error(`${nodeType} schema has no valid defaults:`, fallbackError);
...
export const CommonSchemas = {
	...
	safeHtml: z
		...
		.refine(
			(html) => !/<iframe\b[^<]*(?:(?!<\/iframe>)<[^<]*)*<\/iframe>/gi.test(html),
			"Iframe tags are not allowed"
		),

	/** Numeric value with bounds and finite check */
	number: z.number().finite("Must be a valid number").safe("Number must be within safe range"),
	...
	stringArray: z.array(z.string()).max(1000, "Too many items (max 1,000)").default([]),

	/** JSON object with size validation */
	jsonObject: z
		.record(z.unknown())
		.refine((obj) => JSON.stringify(obj).length <= 100000, "JSON object too large (max 100KB)")
		.default({}),
	...
	/** Email validation */
	email: z.string().email("Must be a valid email").max(254, "Email too long").optional(),
 ...
 */
export function createUpdateValidator<T>(schema: z.ZodSchema<T>, nodeType: string) {
	return function validateUpdate(currentData: T, updates: Partial<T>, nodeId?: string): T {
		try {
