 ...
 * Keywords: code-generation, json-processing, build-time
 */
...
const JsonCategorySchema = z.record(
  z.object({
    displayName: z.string(),
    description: z.string(),
    icon: z.string(),
...
async function loadNodeMetadata(filePath: string): Promise<{
  ...
  const nodeData = await readJsonFile(filePath, JsonNodeSchema);
...
async function loadCategories(metaDir: string): Promise<Record<string, any>> {
  ...
  try {
    ...
    const parsed = JSON.parse(content);
    return JsonCategorySchema.parse(parsed.categories);
  } catch (error) {
