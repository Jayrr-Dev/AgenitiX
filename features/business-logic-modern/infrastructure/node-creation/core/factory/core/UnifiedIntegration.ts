...
import { NodeFactory as BasicNodeFactory } from "../utils/creation/nodeFactory";

// Import inspector control functions from unified registry
import {
...
export interface UnifiedNodeFactory {
  ...
  // INSPECTOR CONTROLS
  getNodeInspectorControls: (nodeType: string) => any;
  hasFactoryInspectorControls: (nodeType: string) => boolean;
...
class UnifiedFactoryImpl implements UnifiedNodeFactory {
  ...
  // INSPECTOR CONTROLS
  /**
   * Get node inspector controls component for a specific node type
   * @param nodeType - The node type to get controls for
   * @returns Inspector controls component or null if not found
   */
  getNodeInspectorControls(nodeType: string): any {
    try {
      return registryGetNodeInspectorControls(nodeType);
    } catch (error) {
      this.handleError("getNodeInspectorControls failed", error);
      return null;
    }
  }

  /**
   * Check if a node type has factory inspector controls registered
   * @param nodeType - The node type to check
   * @returns True if the node type has inspector controls
   */
  hasFactoryInspectorControls(nodeType: string): boolean {
    try {
      return registryHasFactoryInspectorControls(nodeType);
    } catch (error) {
      this.handleError("hasFactoryInspectorControls failed", error);
      return false;
    }
  }
...
export const IntegratedNodeFactory = {
  ...
  getNodeInspectorControls: unifiedFactory.getNodeInspectorControls,
  hasFactoryInspectorControls: unifiedFactory.hasFactoryInspectorControls,
} as const;
