...
import { useComponentTheme, debugComponentColors, getColorInfo } from './componentThemeStore';
...
if (typeof window !== 'undefined' && IS_DEVELOPMENT) {
  ...
  (window as any).debugColors = (componentName?: string) => {
    if (componentName) {
      ...
      import('./componentThemeStore').then(({ debugComponentColors, useComponentTheme }) => {
        // This is a simplified version - in practice you'd need the actual theme
        console.log(`🎨 Debug colors for ${componentName} - use the visual debugger for full details`);
      ...
      console.log('• Available components: actionToolbar, historyPanel, sidePanel, sidebarIcons, variantSelector, nodeInspector, miniMap');
    }
  };
