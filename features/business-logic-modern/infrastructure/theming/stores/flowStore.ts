...
import { INITIAL_EDGES, INITIAL_NODES } from "@flow-editor/constants";
import type { AgenEdge, AgenNode, NodeError } from "@flow-editor/types";
import { performCompleteMemoryCleanup } from "@flow-editor/utils/memoryCleanup";
import {
  cleanupNodeTimers,
  emergencyCleanupAllTimers,
} from "@flow-editor/utils/timerCleanup";
...
export const useSelectedEdge = () => {
  return useFlowStore((state) => {
    ...
    return selectedEdgeId
      ? state.edges.find((e) => e.id === selectedEdgeId) || null
      : null;
  });
};
...
export const useNodeById = (nodeId: string) => {
  return useFlowStore(
    (state) => state.nodes.find((n) => n.id === nodeId) || null
  );
};
