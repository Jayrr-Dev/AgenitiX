...
const HistoryPanel: React.FC<HistoryPanelProps> = ({ className = "" }) => {
  ...
  // Active path id set (root→cursor)
  const activePathIdSet = useMemo(() => {
    const s = new Set<string>();
    for (const n of historyData.entries || []) {
      if (n?.id) s.add(n.id);
    }
    return s;
  }, [historyData.entries]);
  ...
  // GET WINDOWED LINEAR HISTORY (root→cursor only, plus immediate futures)
  const fullLinearHistory = useMemo(() => {
    ...
    const addFutureStates = (nodeId: string) => {
      ...
      for (const childId of (node.childrenIds || []).slice(0, 12)) {
        const childNode = fullGraphData.nodes[childId];
        if (childNode) {
          currentPath.push(childNode);
          // Avoid deep recursion for performance in list view
        }
      }
    };
  ...
  const getActionIcon = useCallback(
    (metadata: Record<string, unknown>): React.ReactNode => {
  ...
  const renderHistoryItem = useCallback(
    (state: HistoryNode, index: number) => {
      ...
      const { itemStyle, indicatorStyle, badgeStyle, textStyle } =
        getStateStyles(index, isSelected);
      ...
      const stateLabel =
        state.label ||
        (typeof state.metadata?.action === "string"
        ...
        >
          <div className={HISTORY_ITEM_STYLES.layout}>
            ...
            <div className={HISTORY_ITEM_STYLES.rightSection}>
              {isCurrentState && (
                <span className={HISTORY_ITEM_META_STYLES.currentBadge}>
                  Current
                </span>
              )}
              {isFutureState && (
  ...
  const handleClearHistory = useCallback(() => {
    ...
    if (selectedNodeId && selectedNodeId !== fullGraphData.root) {
      const ok = removeSelectedNode(selectedNodeId);
      if (!ok) console.warn(`Failed to remove node ${selectedNodeId}`);
    } else {
      clearHistory();
    }
  }, [selectedNodeId, fullGraphData, removeSelectedNode, clearHistory]);

  // Branch pruning tools
  const handlePruneBranch = useCallback(() => {
    if (!fullGraphData || !selectedNodeId) return;
    pruneBranch?.(selectedNodeId);
  }, [fullGraphData, selectedNodeId, pruneBranch]);

  const handlePruneFuture = useCallback(() => {
    pruneFutureFrom?.(fullGraphData?.cursor);
  }, [pruneFutureFrom, fullGraphData?.cursor]);
        ...
        <div className={HEADER_STYLES.layout}>
          ...
          <div className={HEADER_STYLES.rightSection}>
            ...
            <div className={CONTROLS_STYLES.container}>
              ...
              {/* Prune future from cursor */}
              <button
                onClick={handlePruneFuture}
                className={CONTROLS_STYLES.button}
                title="Prune future (clear redo branches)"
                type="button"
                disabled={
                  !fullGraphData ||
                  !fullGraphData.nodes[fullGraphData.cursor]?.childrenIds
                    ?.length
                }
              >
                <Scissors className={CONTROLS_STYLES.icon} />
              </button>

              {/* Prune non-path branch when a non-path node is selected */}
              <button
                onClick={handlePruneBranch}
                className={CONTROLS_STYLES.button}
                title="Prune selected branch"
                type="button"
                disabled={
                  !selectedNodeId ||
                  selectedNodeId === fullGraphData?.root ||
                  (selectedNodeId ? activePathIdSet.has(selectedNodeId) : true)
                }
              >
                <Zap className={CONTROLS_STYLES.icon} />
              </button>
      ...
      <div className={CONTENT_STYLES.container}>
        {graphView ? (
          <div className={CONTENT_STYLES.graphContainer}>
            {fullGraphData && fullGraphData.nodes && fullGraphData.root ? (
              <Suspense
                fallback={
