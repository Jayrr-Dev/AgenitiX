    ...
    <AnimatePresence mode="wait" initial={false}>
      {stencil && (
        <motion.div
          ...
          className="mb-2 px-3 py-2 text-xs leading-snug absolute top-0 right-[50%] translate-x-1/2 border -translate-y-full w-[450px] h-[95px] bg-(--infra-sidebar-bg) border-(--infra-sidebar-border) text-(--infra-sidebar-text) rounded-lg shadow-lg"
        >
          <div className="font-medium text-(--infra-sidebar-text)">
            {stencil.label}
          </div>
          <div className="text-(--infra-sidebar-text-secondary)">
            {stencil.description}
          </div>
        </motion.div>
      )}
