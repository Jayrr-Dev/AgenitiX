...
export const findEdgeByHandle = (
  ...
  return edges.find(
    (e) => e.target === targetId && e.targetHandle?.startsWith(handleBaseName)
  );
};
...
export const findEdgesByHandle = (
  ...
  return edges.filter(
    (e) => e.target === nodeId && e.targetHandle?.startsWith(handleBaseName)
  );
};
 ...
 */
export const getNodesWithRemovedInputs = (
  changes: EdgeChange[],
  edges: Edge[]
): string[] => {
  ...
  const remainingEdges = edges.filter(
    (edge) => !removedEdges.includes(edge.id)
  );
  const nodesWithoutInputs: string[] = [];

  for (const nodeId of affectedNodeIds) {
    const stillHasInputs = remainingEdges.some(
...
export const extractNodeValue = (
  ...
  // Priority chain: output ➜ store ➜ isActive ➜ fallback
  return (
    nodeData.output ?? nodeData.store ?? nodeData.isActive ?? fallbackValue
  );
};
