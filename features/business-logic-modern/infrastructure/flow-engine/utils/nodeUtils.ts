 ...
 *
 * Priority: output > output.result > inputs.value > data itself (as stringified JSON)
 *
...
export const extractNodeValue = (
  ...
  // Check for output first (most common case)
  if (data.output !== undefined && data.output !== null) {
    return data.output;
  }

  // Special handling for AI Agent when output is null - check processingResult
  ...
  // Check for output.result (nested case)
  ...
  // For AI Agent, check processingResult as fallback if output is not available
  if (data.processingResult !== undefined && data.processingResult !== null) {
  ...
  // If output is an object but shouldn't be (data corruption), try to extract text
  if (data.output && typeof data.output === "object" && data.output !== null) {
    ...
    console.warn(
      "AI Agent output field contains object instead of string:",
      outputObj
    );
