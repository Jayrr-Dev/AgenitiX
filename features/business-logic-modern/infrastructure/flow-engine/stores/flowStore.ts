...
// CIRCULAR REFERENCE FIX: Safe serialization for Zustand persistence
const safeSerialize = (state: FlowState) => {
  try {
    ...
    console.error("Failed to serialize flow state:", error);
...
export const useUpdateNodeData = () =>
  useFlowStore((state) => state.updateNodeData);
