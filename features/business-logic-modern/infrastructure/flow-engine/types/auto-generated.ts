...
function generateNodeUnionType(): string {
  const nodeTypes = Object.keys(getNodeTypeConfig()) as NodeType[];
...
export function generateTypeDefinitions(): string {
  ...
  const interfaces = nodeTypes
    .map((nodeType) =>
      generateNodeDataInterface(nodeType, getNodeTypeConfig()[nodeType])
    )
    .join("\n\n");
...
export function getRegisteredNodeTypes(): NodeType[] {
  return Object.keys(getNodeTypeConfig()) as NodeType[];
}
...
export function getNodeDefaultData(nodeType: NodeType) {
  return getNodeTypeConfig()[nodeType].defaultData;
}
