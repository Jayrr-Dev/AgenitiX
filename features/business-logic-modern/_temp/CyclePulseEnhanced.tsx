...
const TimerManager = {
  ...
  startCycle: (
    ...
    updateNodeData: (
      id: string,
      updates: Partial<CyclePulseEnhancedData>
    ) => void
  ) => {
    console.log(
      `CyclePulseEnhanced ${id}: Starting cycle with duration ${data.cycleDuration}ms`
    );
    ...
    const runCycle = () => {
      ...
      const isPulseActive =
        isPulsePhase && elapsed - cycleDuration < pulseDuration;
      ...
      if (isCycleComplete) {
        ...
        const shouldContinue =
          data.infinite || newCycleCount < (data.maxCycles || 1);
...
const CyclePulseEnhanced = createNodeComponent<CyclePulseEnhancedData>({
    ...
    try {
      ...
      if (hasExternalTrigger) {
        ...
        if (isTriggered && !data.isRunning) {
          ...
          TimerManager.startCycle(
            id,
            { ...data, isRunning: true, cycleCount: 0 },
            updateNodeData
          );
      ...
      if (
        !data.infinite &&
        data.cycleCount >= data.maxCycles &&
        data.isRunning
      ) {
        TimerManager.stopCycle(id, data);
        updateNodeData(id, {
      ...
      console.error(
        `CyclePulseEnhanced ${id} - Processing error:`,
        processingError
      );
      const errorMessage =
  ...
  renderCollapsed: ({ data, error, updateNodeData, id }) => {
    ...
    const handleToggle = () => {
      console.log(
        `CyclePulseEnhanced ${id}: Manual toggle, current isRunning:`,
        data.isRunning
      );
  ...
  renderExpanded: ({ data, error, categoryTextTheme, updateNodeData, id }) => {
    ...
    return (
      <div className="flex flex-col w-full h-full text-xs p-1">
        <div
          className={`font-semibold mb-1 text-center ${categoryTextTheme.primary}`}
        >
          {finalError ? (
        ...
        <div className="flex-1 space-y-1 min-h-0">
          ...
          <div className="border-t border-gray-200 dark:border-gray-600 pt-1 mt-1">
            <div className="flex items-center gap-1">
              <input
                ...
                onChange={(e) =>
                  updateNodeData(id, { autoStart: e.target.checked })
                }
                disabled={!!finalError}
                className="shrink-0"
              />
            ...
            <div className="flex items-center gap-1">
              <input
                ...
                onChange={(e) =>
                  updateNodeData(id, { burstMode: e.target.checked })
                }
                disabled={!!finalError}
                className="shrink-0"
              />
              ...
              <div className="flex items-center gap-1 ml-4">
                ...
                <input
                  ...
                  onChange={(e) => {
                    ...
                    const numValue = parseInt(value) || 1;
                    updateNodeData(id, {
                      burstCount: Math.max(1, Math.min(10, numValue)),
                    });
                  }}
          ...
          {/* STATUS DISPLAY */}
          <div
            className={`text-xs ${categoryTextTheme.secondary} text-center py-1`}
          >
            {data.isRunning ? (
...
export const cleanupCyclePulseEnhancedTimers = (
  id: string,
  data: CyclePulseEnhancedData
) => {
  TimerManager.stopCycle(id, data);
