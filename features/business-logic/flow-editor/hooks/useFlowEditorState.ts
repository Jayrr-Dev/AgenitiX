...
import type { AgenNode, AgenEdge, NodeError, FlowEditorState } from '../types';
import { INITIAL_NODES, INITIAL_EDGES, MAX_ERRORS_PER_NODE } from '../constants';
...
export function useFlowEditorState() {
  ...
  const updateNodeId = useCallback((oldId: string, newId: string) => {
    ...
    setNodes((nds) =>
      nds.map((n) =>
        n.id === oldId ? ({ ...n, id: newId } as AgenNode) : n
      )
    );
  ...
  const pasteNodes = useCallback((offset: { x: number; y: number } = { x: 40, y: 40 }) => {
