    ...
    return (
      <div className="absolute inset-0 flex flex-col items-center justify-center px-2">
        ...
        {finalError && errorStyle ? (
          ...
          >
            <CreateTextRefactorInput data={data} updateNodeData={updateNodeData} id={id} />
          </div>
        )}
      </div>
    );
    ...
    return (
      <div className="flex text-xs flex-col w-auto">
        <div className={`font-semibold mb-2 flex items-center justify-between ${categoryTextTheme.primary}`}>
          <span>{finalError ? `${finalErrorType === 'local' ? 'Error' : finalErrorType.toUpperCase()}` : 'Create Text (Refactored)'}</span>
          {finalError && errorStyle && (
...
const CreateTextRefactorInput = ({ data, updateNodeData, id }: { 
...
const CreateTextRefactorExpanded = ({ data, error, errorStyle, categoryTextTheme, updateNodeData, id }: { 
  data: CreateTextRefactorData; 
  error: string | null; 
