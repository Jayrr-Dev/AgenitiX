...
const DataTypeManager = {
  ...
  shouldIncludeValue: (
    ...
    if (data.filterDuplicates && previousValue !== undefined) {
      try {
        if (JSON.stringify(content) === JSON.stringify(previousValue))
          return false;
      } catch {
...
const ViewOutputEnhanced = createNodeComponent<ViewOutputEnhancedData>({
  ...
  processLogic: ({
    ...
    try {
      ...
      const extractedValues = nodesData.map((node) => {
      ...
      for (let i = 0; i < extractedValues.length; i++) {
        ...
        const previousContent =
          i > 0 ? extractedValues[i - 1].content : undefined;

        if (
          DataTypeManager.shouldIncludeValue(
            item.content,
            data,
            previousContent
          )
      ...
      const hasChanged =
        limitedValues.length !== currentValues.length ||
      ...
      console.error(
        `ViewOutputEnhanced ${id} - Processing error:`,
        processingError
      );
      const errorMessage =
          ...
          {finalError ? (
            ...
            <div className="space-y-1 h-full overflow-hidden">
              {values.slice(0, 4).map((item, index) => {
                ...
                return (
                  <div
                    key={`${item.id}-${item.type}-${index}-collapsed`}
                    className="flex items-center gap-1 bg-white/70 dark:bg-black/30 rounded px-1 py-0.5"
                  >
                    {data.showTypeIcons && (
                      <div
        ...
        {!finalError ? (
          ...
          >
              ...
              <div className="flex items-center gap-2 text-xs">
                <label className="flex items-center gap-1">
                  <input
                    ...
                    onChange={(e) =>
                      updateNodeData(id, { filterEmpty: e.target.checked })
                    }
                    className="shrink-0"
                  />
                  <span className={categoryTextTheme.secondary}>
                    Filter empty
                  </span>
                </label>
                <label className="flex items-center gap-1">
                  <input
                    ...
                    onChange={(e) =>
                      updateNodeData(id, { showTypeIcons: e.target.checked })
                    }
                    className="shrink-0"
                  />
            ...
            >
              {values.length === 0 ? (
                <div
                  className={`text-xs italic ${categoryTextTheme.secondary} text-center py-4`}
                >
                  ...
                  return (
                    ...
                    >
                      ...
                      <div className="flex items-center gap-2 mb-1">
                        ...
                        <span
                          className={`text-xs font-medium ${categoryTextTheme.primary}`}
                        >
                          {typeInfo.label}
                          {typeInfo.size !== undefined && ` (${typeInfo.size})`}
                        </span>
                        <span
                          className={`text-xs ${categoryTextTheme.secondary} ml-auto font-mono`}
                        >
                          {item.id.slice(-4)}
                        </span>
                      </div>
                      ...
                      <div
                        className={`text-xs font-mono break-all ${categoryTextTheme.secondary}`}
                      >
