...
// -----------------------------------------------------------------------------
const LogicAnd: React.FC<
  NodeProps<Node<LogicAndData & Record<string, unknown>>>
> = ({ id, data }) => {
  ...
  const andResult =
    ...
    inputValues.every((value) => isTruthyValue(value));
  ...
  return (
    <div
      className={`relative ${showUI ? "px-4 py-3 min-w-[120px] min-h-[120px]" : "w-[60px] h-[60px] flex items-center justify-center"} rounded-lg bg-lime-100 dark:bg-lime-900 shadow border border-lime-300 dark:border-lime-800`}
    >
      {/* Floating Node ID */}
      ...
      {!showUI && (
        <div className="absolute inset-0 flex items-center justify-center">
          <div className="w-16 h-16 flex items-center justify-center">
            <div
              className={`text-lg font-bold ${andResult ? "text-lime-700 dark:text-lime-300" : "text-lime-400 dark:text-lime-600"}`}
            >
              AND
            </div>
          </div>
      ...
      {showUI && (
        <>
          <div className="font-semibold text-lime-900 dark:text-lime-100 mb-2">
            AND
          </div>
