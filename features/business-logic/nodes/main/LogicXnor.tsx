...
// -----------------------------------------------------------------------------
const LogicXnor: React.FC<NodeProps<Node<LogicXnorData & Record<string, unknown>>>> = ({ id, data }) => {
  ...
  const xnorResult = totalInputs > 0 && (trueInputs === 0 || trueInputs === totalInputs)
  ...
  // RENDER
  return (
    <div className={`relative ${showUI ? 'px-4 py-3 min-w-[120px] min-h-[120px]' : 'w-[60px] h-[60px] flex items-center justify-center'} rounded-lg bg-cyan-100 dark:bg-cyan-900 shadow border border-cyan-300 dark:border-cyan-800`}>
      ...
      {!showUI && (
        <div className="absolute inset-0 flex items-center justify-center">
          <div className="w-16 h-16 flex items-center justify-center">
            <div className={`text-sm font-bold ${xnorResult ? 'text-cyan-700 dark:text-cyan-300' : 'text-cyan-400 dark:text-cyan-600'}`}>
              XNOR
            </div>
          </div>
        </div>
      ...
      {showUI && (
        <>
          <div className="font-semibold text-cyan-900 dark:text-cyan-100 mb-2">XNOR</div>
