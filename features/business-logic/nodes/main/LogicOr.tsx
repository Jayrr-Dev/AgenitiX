...
// -----------------------------------------------------------------------------
const LogicOr: React.FC<
  NodeProps<Node<LogicOrData & Record<string, unknown>>>
> = ({ id, data }) => {
  ...
  return (
    <div
      className={`relative ${showUI ? "px-4 py-3 min-w-[120px] min-h-[120px]" : "w-[60px] h-[60px] flex items-center justify-center"} rounded-lg bg-yellow-100 dark:bg-yellow-900 shadow border border-yellow-300 dark:border-yellow-800`}
    >
      {/* Floating Node ID */}
      ...
      {!showUI && (
        <div className="absolute inset-0 flex items-center justify-center">
          <div className="w-16 h-16 flex items-center justify-center">
            <div
              className={`text-lg font-bold ${orResult ? "text-yellow-700 dark:text-yellow-300" : "text-yellow-400 dark:text-yellow-600"}`}
            >
              OR
            </div>
          </div>
