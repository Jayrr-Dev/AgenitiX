...
const TriggerOnToggle: React.FC<
  NodeProps<Node<TriggerOnToggleData & Record<string, unknown>>>
> = ({ id, data }) => {
  ...
  return (
    <div
      className={`relative ${showUI ? "px-4 py-3 min-w-[180px] min-h-[120px]" : "w-[60px] h-[60px] flex items-center justify-center"} rounded-lg bg-violet-100 dark:bg-violet-900 shadow border border-violet-300 dark:border-violet-800`}
    >
      ...
      {!showUI && (
        <div className="absolute inset-0 flex items-center justify-center">
          <IconForToggle isOn={triggered} onClick={handleToggle} size={40} />
        </div>
      )}
      ...
      {showUI && (
        <div className="flex flex-col items-center">
          ...
          <IconForToggle isOn={triggered} onClick={handleToggle} size={48} />
          <div className="text-xs text-violet-800 dark:text-violet-200 mt-2">
            Status:{" "}
            <span className="font-mono">{triggered ? "ON" : "OFF"}</span>
          </div>
