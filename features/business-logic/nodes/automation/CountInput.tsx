...
const isTrueValue = (value: unknown): boolean => {
  ...
  if (typeof value === "object" && value !== null) {
    ...
    if ("value" in obj && typeof obj.value === "boolean")
...
const CounterNode: React.FC<
  NodeProps<Node<CounterNodeData & Record<string, unknown>>>
> = ({ id, data }) => {
  ...
  const currentInputValues = useMemo(() => {
    ...
    inputConnections.forEach((connection, index) => {
      ...
      if ("outputValue" in nodeData)
        values[nodeId] = nodeData.outputValue; // DelayNode and other nodes with outputValue
      else if ("triggered" in nodeData)
        values[nodeId] = nodeData.triggered; // Trigger nodes
      else if ("value" in nodeData)
        values[nodeId] = nodeData.value; // Logic/Converter nodes
      else if ("text" in nodeData)
        values[nodeId] = nodeData.text; // Text nodes
      else if ("count" in nodeData)
        values[nodeId] = nodeData.count; // Counter nodes
  ...
  const [localMultiplierInput, setLocalMultiplierInput] = useState(
    multiplier.toString()
  );
  ...
  const prevInputValues = useRef<Record<string, unknown>>(
    data.lastInputValues ?? {}
  );
  const isFirstRender = useRef(true);
  const lastSyncedCount = useRef(data.count ?? 0);
  const lastSyncedMultiplier = useRef(data.multiplier ?? 1);
  const updateTimeoutRef = useRef<ReturnType<typeof setTimeout> | null>(null);
  const autoCountIntervalRef = useRef<ReturnType<typeof setInterval> | null>(
    null
  );
  ...
  useEffect(() => {
    ...
    if (
      ...
      data.count !== count &&
      data.count !== lastSyncedCount.current
    ) {
      setCount(data.count);
    ...
    if (
      ...
      data.multiplier !== lastSyncedMultiplier.current
    ) {
    ...
    if (
      typeof data.autoCount === "boolean" &&
      data.autoCount !== autoCount &&
      data.autoCount !== lastSyncedAutoCount.current
    ) {
    ...
    if (
      typeof data.countSpeed === "number" &&
      data.countSpeed !== countSpeed &&
      data.countSpeed !== lastSyncedCountSpeed.current
    ) {
  ...
  const handleMultiplierKeyDown = useCallback(
  ...
  return (
    <div
      className={`relative ${showUI ? "px-4 py-3 min-w-[200px] min-h-[180px]" : "w-[80px] h-[80px] flex items-center justify-center"} rounded-lg shadow border bg-blue-100 dark:bg-blue-900 border-blue-300 dark:border-blue-800`}
    >
        ...
        <>
          ...
          <div className="flex flex-col gap-2 mb-3 w-full">
            <div className="flex items-center gap-2">
              ...
              />
              <label
                htmlFor={`auto-count-${id}`}
                className="text-xs text-blue-700 dark:text-blue-300 cursor-pointer"
              >
                Auto Count
              </label>
              {autoCount && (
          ...
          {inputConnections.length > 0 && (
            <div className="text-xs text-blue-600 dark:text-blue-400">
              Auto-counting on true values ({inputConnections.length} input
              {inputConnections.length !== 1 ? "s" : ""})
            </div>
          )}
        </>
      )}
