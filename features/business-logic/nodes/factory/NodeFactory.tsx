...
export function createNodeComponent<T extends BaseNodeData>(
  ...
  const NodeComponent = ({
    ...
    const { propagateUltraFast, enableGPUAcceleration } =
      useUltraFastPropagation(allNodes || [], connections, updateNodeData);
    ...
    const processJsonInputs = useCallback(() => {
      if (
        !jsonProcessingTracker.current.shouldProcess(PROCESSING_THROTTLE_MS)
      ) {
        return;
      }
    ...
    useEffect(() => {
      const hasJsonHandles = enhancedConfig.handles.some(
    ...
    const calculatedIsActive = useMemo(() => {
      try {
        ...
        if (isHead) {
          return calculateHeadNodeActivation(
            enhancedConfig.nodeType,
            data as T,
            bypassCache
          );
        ...
        console.error(
          `${enhancedConfig.nodeType} ${id} - Propagation calculation error:`,
          error
        );
        return false;
      }
    ...
    useEffect(() => {
      if (isActive !== calculatedIsActive) {
        ...
        if (isDeactivating) {
          console.log(
            `UFS ${enhancedConfig.nodeType} ${id}: DEACTIVATING - Using ultra-fast instant propagation`
          );
        } else if (isActivating) {
          console.log(
            `UFS ${enhancedConfig.nodeType} ${id}: ACTIVATING - Using ultra-fast smooth propagation`
          );
        }
    ...
    useEffect(() => {
      ...
      try {
        ...
        if (
          enhancedConfig.nodeType.toLowerCase().includes("trigger") ||
          ...
          if (currentOutputValue !== outputValue) {
            ...
            updateNodeData(id, { value: outputValue } as Partial<
              Record<string, unknown>
            >);

            if (isOutputDeactivating) {
              console.log(
                `UFS Output ${enhancedConfig.nodeType} ${id}: INSTANT output deactivation`
              );
        ...
        if ((data as any)?.isActive === true) {
          updateNodeData(id, { isActive: false } as Partial<
            Record<string, unknown>
          >);
        }

        console.error(
          `${enhancedConfig.nodeType} ${id} - Processing error:`,
          processingError
        );
        const errorMessage =
    ...
    const recoverFromError = useCallback(() => {
      try {
        ...
        console.error(
          `${enhancedConfig.nodeType} ${id} - Recovery failed:`,
          recoveryError
        );
    ...
    const errorState: ErrorState = useMemo(() => {
      ...
      const hasVibeError =
        supportsErrorInjection && (data as any)?.isErrorState === true;
    ...
    const categoryBaseClasses = useNodeCategoryBaseClasses(
      enhancedConfig.nodeType
    );
        ...
        const inputHandlesFiltered = enhancedConfig.handles
          ...
          .filter((handle) => {
            if (handle.dataType === "j") {
              ...
              return (
                hasJsonConnection || // Priority 1: Connected = always visible
                showJsonHandles || // Priority 2: X button state
                isVibeModeActive || // Priority 3: Vibe Mode legacy
                hasJsonSources
              ); // Priority 4: Smart auto-show
            }
            return true;
          });

        const outputHandles = enhancedConfig.handles.filter(
      ...
      >
        ...
        {!showUI &&
          enhancedConfig.renderCollapsed({
            data: data as T,
            error:
              errorState.supportsErrorInjection &&
              errorState.finalErrorForStyling
                ? error || (data as any)?.error || "Error state active"
                : error,
