...
export const useTimesheetStore = create<TimesheetStore>()(
  persist(
    (set, get) => ({
      ...
      updateData: (data: UpdateData[]) => set((state) => ({
        data: state.data.map(row => 
          data.some(update => update.id === row.id) ? { ...row, ...data.find(update => update.id === row.id)?.updatedRow } : row
        )
      })),
      ...
      // Update a row
      updateRow: (id, updatedRow) => set((state) => ({
        data: state.data.map(row => 
      ...
      markRowAsSynced: (clientId, serverId) => set((state) => ({
        data: state.data.map(row => 
      ...
      // Undo/redo history
      history: [],
      addToHistory: (prevData) => set((state) => ({
