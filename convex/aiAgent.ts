...
const safeCalculate = (expression: string): number => {
  ...
  const parseTerm = (): number => {
    ...
    while (
      pos < cleanExpr.length &&
      (cleanExpr[pos] === "*" || cleanExpr[pos] === "/")
    ) {
...
const performWebSearch = async (
  ...
  try {
    ...
    if (TAVILY_API_KEY) {
      ...
      // Tavily API key not found, skipping to DuckDuckGo
    }

    // Strategy 2: Fallback to DuckDuckGo Instant Answer API
    try {
      // Attempting DuckDuckGo search
      ...
      if (duckGoResponse.ok) {
        ...
        if (
          duckGoData.RelatedTopics &&
          Array.isArray(duckGoData.RelatedTopics)
        ) {
          duckGoData.RelatedTopics.slice(0, maxResults).forEach(
            (topic: any) => {
        ...
        if (hasResults) {
          ...
          // Returning DuckDuckGo search results
          return result;
        }
      }
    } catch (duckGoError) {
      // DuckDuckGo search failed, trying Wikipedia
    }
    ...
    try {
      ...
      const isFactualQuery = query
        .toLowerCase()
        .match(
          /(what is|who is|when was|where is|how does|definition|meaning|history|bitcoin|cryptocurrency|climate|science|technology|medicine|geography|biography)/
        );
      ...
      if (isFactualQuery) {
        ...
        const searchTerm = query
          .replace(
            /what is|who is|when was|where is|how does|definition of|meaning of|history of/gi,
            ""
          )
          .trim()
        ...
        if (wikiResponse.ok) {
          ...
          // Returning Wikipedia search results
          return result;
        }
      }
    } catch (wikiError) {
    ...
    return (
      `Unable to perform web search for "${query}" at this time.\n\n` +
      `Reason: ${error}\n\n` +
...
const getEnabledTools = (
  enabledTools: Array<{ type: string; name: string; config: any }>
) => {
  const tools: any = {};
      ...
      switch (tool.type) {
        case "calculator":
          ...
          tools.calculator = createTool({
            description:
              "Calculate mathematical expressions using the built-in calculator. ALWAYS use this tool instead of writing code when the user asks for calculations. This tool will compute the result directly.",
            args: z.object({
              expression: z
                .string()
                .describe(
                  "Mathematical expression to evaluate (e.g., '2 + 3 * 4', '(10 + 5) / 3', '123456789 * 987654321')"
                ),
            }),
            handler: async (ctx, args) => {
              // Calculator tool called with args
              try {
        ...
        case "webSearch":
          ...
          tools.webSearch = createTool({
            description:
              "Search the web for ANY information, including current news, facts, Wikipedia articles, recent events, prices, weather, and general knowledge. This tool can find information from all websites including Wikipedia, news sites, and other sources. Use this whenever you need to look up information that might not be in your training data or when the user asks for current/recent information.",
            args: z.object({
              query: z
                .string()
                .describe(
                  "Search query to find information on the web (e.g., 'latest AI trends 2025', 'Bitcoin price today', 'weather in New York')"
                ),
              maxResults: z
                .number()
                .optional()
              ...
              includeAnswer: z
                ...
                .optional()
                .describe(
                  "Whether to include a quick answer if available (default: true)"
                ),
            }),
            handler: async (ctx, args) => {
              // Web search tool called with args
              try {
...
export const processUserMessage = action({
  ...
  handler: async (ctx, args): Promise<AiAgentMessage> => {
    try {
      // Processing user message with config
      ...
      if (threadId && threadId.length > 10 && !threadId.includes("_")) {
        try {
          ...
          // Failed to continue thread, creating new one
  ...
  handler: async (ctx, args) => {
    try {
      const agent = new Agent(components.agent, {
        chat: openai.chat("gpt-3.5-turbo"),
      });
  ...
  handler: async (ctx, args) => {
    try {
      ...
      const testAgent = new Agent(components.agent, {
        ...
        chat: chatModel,
        instructions:
          "You are a test agent. Respond with 'OK' to test messages.",
        maxSteps: 1,
      });

      // Create a test thread
      const { thread } = await testAgent.createThread(ctx, {
        userId: "test-user",
      });
