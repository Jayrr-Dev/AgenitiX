    ...
    if (debouncedSearch) {
      ...
      filtered = filtered.filter(
        (flow) =>
          ...
          flow.icon?.toLowerCase().includes(searchLower) ||
              ...
              <div>
                ...
                {debugInfo && (
                  <div className="text-xs text-muted-foreground mt-2">
                    Debug: {debugInfo.totalFlows} total, {debugInfo.publicFlows}{" "}
                    public, {debugInfo.privateFlows} private
                    {simpleFlows &&
                      ` | Simple query: ${simpleFlows.length} flows`}
                  </div>
                )}
              </div>
